...
struct GymView: View {
  ...
  var body: some View {
    GainsScreen {
      VStack(alignment: .leading, spacing: GainsSpacing.l) {
        ...
        switch selectedTab {
        case .heute:
          // 2026-05-15 (Audit-Loop 14): `showsPlanWizard`-Binding entfernt
          // — GymTodayTab hat es nie gelesen.
          GymTodayTab(
    ...
    .sheet(isPresented: $isShowingWorkoutBuilder) {
      // 2026-05-15 (P1 #3): onSaved-Callback spiegelt HomeView-Verhalten —
  ...
  private var tabPicker: some View {
      ...
      ForEach(Array(GymTab.allCases.enumerated()), id: \.element) { index, tab in
        ...
        Button {
          ...
          Text(tab.rawValue)
            ...
            .background(
              // 2026-05-14 (Polish-Loop 46): Aktive Pille mit Inner-
              // Light + Bottom-Dim — gleiche Sprache wie SegmentedPicker
              // und Primary-Button.
              Group {
                if isActive {
                  ZStack {
                  ...
                  .compositingGroup()
                  .shadow(color: GainsColor.lime.opacity(0.16), radius: 8)
                } else {
                  RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
                    .fill(Color.clear)
                }
    ...
    .background(
      // 2026-05-14 (Polish-Loop 47): Glas-Container statt flacher Card.
      ZStack {
