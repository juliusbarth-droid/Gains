  ...
  private var targetDetailRow: some View {
    ...
    case .free:
      Text(selectedModality.isCycling
        ? "Freie Fahrt mit normalem Live-Tracking, nur ohne festes Distanz-, Zeit- oder Tempo-Ziel."
        : "Freier Lauf mit normalem Live-Tracking, nur ohne festes Distanz-, Zeit- oder Pace-Ziel.")
        .font(GainsFont.body(13))
        .foregroundStyle(GainsColor.softInk)
...
private struct LiveRunView: View {
  ...
  private var displayedAnnotationCoordinate: CLLocationCoordinate2D? {
    if isTrackerActive, !run.isPaused, !gpsTracker.autoPaused {
      return gpsTracker.currentCoordinate
    }
...
private struct StopRunSheet: View {
  ...
  private var activityLapLabel: String {
    (run?.modality.isCycling ?? false) ? "Abschnitte" : "Runden"
  }
  ...
  var body: some View {
    NavigationStack {
      ...
      .confirmationDialog(
        ...
        Text(isAutoPaused
          ? "Wenn du \(activityAccusative) behältst, kehrst du direkt zu der automatisch pausierten \(activityNoun) zurück und kannst sie weiterführen oder speichern. Wenn du sie verwirfst, wird die automatisch pausierte \(activityNoun) verworfen und Distanz, \(activitySecondaryMetricLabel) sowie \(activityLapLabel) landen nicht im Feed oder in den Routen."
          : (run?.isPaused == true
              ? "Wenn du \(activityAccusative) behältst, kehrst du direkt zu der pausierten \(activityNoun) zurück und kannst sie fortsetzen oder speichern. Wenn du sie verwirfst, wird die pausierte \(activityNoun) verworfen und Distanz, \(activitySecondaryMetricLabel) sowie \(activityLapLabel) landen nicht im Feed oder in den Routen."
              : "Wenn du \(activityAccusative) behältst, kehrst du direkt zu der aktiven \(activityNoun) zurück und kannst sie fortsetzen oder speichern. Wenn du sie verwirfst, wird die aktive \(activityNoun) verworfen und Distanz, \(activitySecondaryMetricLabel) sowie \(activityLapLabel) landen nicht im Feed oder in den Routen."))
      }
      .onAppear {
