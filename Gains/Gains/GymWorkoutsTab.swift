...
struct GymWorkoutsTab: View {
  ...
  // 2026-05-16 Welle-Programme: Vorlagen werden nach Programm-Bucket
  // gruppiert (Upper/Lower, PPL, Full Body, Bro-Split, Klassisch).
  // Collapsed-State per Programm — Default ist alles eingeklappt, der User
  // tippt eine Sektion auf, sobald er sie braucht. Bei aktiver Suche
  // wird die Gruppierung umgangen (flacher Treffer-Stream).
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.m) {
      ...
      if !templates.isEmpty && selectedFilter != .custom {
        templatesSection(templates, hasCustomSectionAbove: !custom.isEmpty)
      }
    }
    .alert(
  ...
  private var header: some View {
    HStack(alignment: .firstTextBaseline) {
      ...
      Menu {
        ...
        // Polish-Loop 173 (2026-05-14): Sort-Menu-Trigger als Glas-Pille
        // statt flachem GainsColor.card. Mini chevron.down deutet das
        // Menu an.
        ...
        .overlay(
          Capsule().strokeBorder(
            LinearGradient(
              colors: [GainsColor.border.opacity(0.75), GainsColor.border.opacity(0.30)],
              startPoint: .top,
              endPoint: .bottom
            ),
  ...
  private var searchBar: some View {
    ...
    .frame(height: 44)
    .background(
      // 2026-05-14 (Polish-Loop 71): Search-Bar mit Glas-Look + leichtem
      // Inner-Light. Macht sie als Eingabefeld erkennbar, ohne laut zu
      // werden.
      ZStack {
  ...
  // Polish-Loop 170 (2026-05-14): Filter-Pillen mit Inner-Light auf
  // aktiver Pille (analog timeRangeFilter in GymStatsTab Loop 116) +
  // Glas-Hairline auf inaktiver Pille.
      ...
      ForEach(SourceFilter.allCases) { filter in
        ...
        Button {
          ...
          Text(filter.rawValue)
            ...
            .overlay(
              Capsule().strokeBorder(
                isActive ? Color.clear : GainsColor.border.opacity(0.55),
                lineWidth: GainsBorder.hairline
              )
            )
            .clipShape(Capsule())
            .compositingGroup()
            .shadow(color: isActive ? GainsColor.lime.opacity(0.28) : .clear, radius: 7)
        }
        .buttonStyle(.plain)
      }
      Spacer()
  ...
  private func sorted(_ plans: [WorkoutPlan]) -> [WorkoutPlan] {
    ...
    case .lastPerformed:
      ...
      // Precompute once: O(n_history) statt O(n_plans × log(n_plans) × n_history).
      let dateMap = lastPerformedDateMap()
      return plans.sorted { lhs, rhs in
        let lhsDate = dateMap[lhs.title]
        let rhsDate = dateMap[rhs.title]
  ...
  /// Baut einmalig ein [title → latestDate]-Dict aus der gesamten Workout-Historie.
  /// O(n_history) — deutlich billiger als O(n_plans × n_history) per-sort-comparator.
  private func lastPerformedDateMap() -> [String: Date] {
    var map: [String: Date] = [:]
    for workout in store.workoutHistory {
      let t = workout.title
      if let existing = map[t] {
        if workout.finishedAt > existing { map[t] = workout.finishedAt }
      } else {
        map[t] = workout.finishedAt
      }
    }
    return map
  }
  ...
  private func workoutRow(_ plan: WorkoutPlan) -> some View {
    ...
    return HStack(spacing: GainsSpacing.s) {
      ...
      Button {
        ...
        // mit Inner-Light + Glow. Locked-State (Blocked) bleibt visuell ruhig.
        ZStack {
          Circle()
            .fill(isBlocked ? GainsColor.background : GainsColor.lime)
          if !isBlocked {
            Circle()
              .fill(
                LinearGradient(
          ...
          Image(systemName: isActive ? "play.fill" : "arrow.right")
            ...
            .offset(x: isActive ? 1 : 0)
        }
        .frame(width: 38, height: 38)
        .overlay(
          Circle()
            .strokeBorder(
              isBlocked ? GainsColor.border.opacity(0.45) : GainsColor.lime.opacity(0.55),
              lineWidth: GainsBorder.hairline
            )
        )
        .compositingGroup()
  ...
  // MARK: - Templates-Section (programm-gruppiert)
  //
  // Bei aktiver Suche: flache Trefferliste, weil das User-Intent „finde
  // dieses Workout" ist und die Programm-Buckets dann nur stören.
  // Sonst: pro Programm-Bucket (Upper/Lower 4-Tage, PPL, Full Body
  // 3-Tage, Bro-Split 5-Tage, „Klassisch" für Plans ohne Programm-Tag)
  // ein collapsible Sub-Header. Default ist alles eingeklappt — User
  // tippt ein Programm auf, wenn er es braucht.

  @ViewBuilder
  private func templatesSection(_ templates: [WorkoutPlan], hasCustomSectionAbove: Bool) -> some View {
    let isSearching = !searchText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    ...
    VStack(alignment: .leading, spacing: GainsSpacing.tight) {
      Text("VORLAGEN")
        ...
        .tracking(2.0)
        .foregroundStyle(GainsColor.softInk)
        .padding(.top, hasCustomSectionAbove ? 6 : 0)

      if isSearching {
        // Flache Liste — User sucht gezielt.
        ForEach(templates) { plan in
          workoutRow(plan)
        }
      } else {
        ForEach(programGroups(for: templates)) { bucket in
          programGroup(name: bucket.name, plans: bucket.plans)
        }
      }
    }
  }

  /// Eindeutig identifizierbarer Programm-Bucket für den ForEach —
  /// ein Tuple direkt in ForEach zu stecken funktioniert in einigen
  /// Swift-Versionen, aber ein named struct ist robuster.
  private struct ProgramBucket: Identifiable {
    let name: String
    let plans: [WorkoutPlan]
    var id: String { name }
  }

  /// Sortiert Vorlagen in Buckets: bekannte Programme in fester
  /// Reihenfolge (passend zur Studienlage), „Klassisch" für Plans
  /// ohne `program`-Tag (Legacy + Custom-Templates falls jemand mal
  /// eine speichert). Leere Buckets werden ausgefiltert.
  private func programGroups(for templates: [WorkoutPlan]) -> [ProgramBucket] {
    let order: [String] = [
      WorkoutPlan.Program.upperLower,
      WorkoutPlan.Program.pushPullLegs,
      WorkoutPlan.Program.fullBody,
      WorkoutPlan.Program.broSplit,
    ]
    var buckets: [String: [WorkoutPlan]] = [:]
    for plan in templates {
      let key = plan.program ?? "Klassisch"
      buckets[key, default: []].append(plan)
    }
    var result: [ProgramBucket] = []
    for name in order {
      if let plans = buckets[name], !plans.isEmpty {
        result.append(ProgramBucket(name: name, plans: plans))
      }
    }
    if let legacy = buckets["Klassisch"], !legacy.isEmpty {
      result.append(ProgramBucket(name: "Klassisch", plans: legacy))
    }
    return result
  }

  @ViewBuilder
  private func programGroup(name: String, plans: [WorkoutPlan]) -> some View {
    let isExpanded = expandedPrograms.contains(name)

    VStack(alignment: .leading, spacing: GainsSpacing.tight) {
      Button {
        withAnimation(.spring(response: 0.28, dampingFraction: 0.88)) {
          if isExpanded {
            expandedPrograms.remove(name)
          } else {
            expandedPrograms.insert(name)
          }
        }
      } label: {
        HStack(spacing: GainsSpacing.xs) {
  ...
  private var emptyResult: some View {
    // 2026-05-14 (Empty-State-Sweep): Beide States bekommen einen CTA.
    EmptyStateView(
      style: .inline,
      ...
      actionLabel: searchText.isEmpty ? "Workout erstellen" : "Suche zurücksetzen",
      action: {
        if searchText.isEmpty {
          isShowingWorkoutBuilder = true
        } else {
