...
struct BarcodeScannerSheet: View {
  ...
  /// Jedes Mal erhöht, wenn der User „Erneut scannen" antippt. Signalisiert
  /// der Camera-Preview, `lastDetected` zu leeren — sonst kann derselbe
  /// Barcode kein zweites Mal erkannt werden.
  @State private var scanID: Int = 0
  ...
  private var scannerView: some View {
    ZStack {
      // Camera preview
      BarcodeCameraPreview(torchOn: $torchOn, scanID: scanID) { barcode in
      ...
      VStack {
        ...
        RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
          ...
          .shadow(color: GainsColor.lime.opacity(0.2), radius: 20)
        ...
        VStack(spacing: GainsSpacing.l) {
          ...
          VStack(spacing: GainsSpacing.s) {
            ...
            HStack(spacing: 0) {
              ...
              Rectangle().fill(GainsColor.border.opacity(0.5)).frame(width: 1, height: 44)
              nutritionCell("\(Int(result.carbsPer100g))g", unit: "", label: "Kohlenhydr.", color: GainsColor.accentCool)
              Rectangle().fill(GainsColor.border.opacity(0.5)).frame(width: 1, height: 44)
              nutritionCell("\(Int(result.fatPer100g))g", unit: "", label: "Fett", color: GainsColor.macroFat)
            }
          }
          ...
          // Polish-Loop 194 (2026-05-14): „Erneut scannen"-CTA mit Glas
          // statt flachem GainsColor.elevated; Hairline-Gradient als Border.
          Button {
            scanID += 1
            ...
            .overlay(
              RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
                .strokeBorder(
                  LinearGradient(
                    colors: [GainsColor.border.opacity(0.65), GainsColor.border.opacity(0.25)],
                    startPoint: .top,
                    endPoint: .bottom
                  ),
  ...
  var body: some View {
    VStack(spacing: GainsSpacing.m) {
      ...
      Button {
        ...
        HStack {
          Image(systemName: isLogged ? "checkmark.circle.fill" : "checkmark")
          Text(isLogged ? "Geloggt" : "Eintragen — \(nutrition.kcal) kcal")
            .font(GainsFont.label(15))
        }
...
struct BarcodeCameraPreview: UIViewRepresentable {
  ...
  func updateUIView(_ uiView: BarcodePreviewView, context: Context) {
    ...
    uiView.resetDetectionIfNeeded(scanID: scanID)
  }
}
...
final class BarcodePreviewView: UIView {
  ...
  deinit {
    // Fix 2026-05-16: AVCaptureSession muss explizit gestoppt werden —
    // sonst hält iOS die Kamera- und Mikrofon-Ressourcen aktiv, obwohl
    // das Sheet bereits geschlossen ist (Batterie-Drain, Camera-Indicator
    // bleibt an, nachfolgende AVSession-Anfragen können fehlschlagen).
    if captureSession?.isRunning == true {
      DispatchQueue.global(qos: .userInitiated).async { [session = captureSession] in
        session?.stopRunning()
      }
    }
  ...
  /// Löscht `lastDetected`, wenn der User eine neue Scan-Runde startet
  /// (erkannt am erhöhten `scanID`). Ohne dieses Reset würde ein bereits
  /// erkannter Barcode beim erneuten Scannen stumm ignoriert.
  func resetDetectionIfNeeded(scanID: Int) {
    guard scanID != lastScanID else { return }
    lastScanID = scanID
    lastDetected = nil
  }
