...
struct ProgressContentView: View {
  ...
  private var coachInsightCard: some View {
    ...
    .accessibilityValue(
      canCycle
        ? "Hinweis \(safeIndex + 1) von \(insights.count). \((current.text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Kein Hinweistext" : current.text))"
        : (current.text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Kein Hinweistext" : current.text)
    )
  ...
  private func historyAccessibilityValue(_ entry: HistoryEntry) -> String {
    ...
    case .workout(let w):
      ...
      return String(format: "Volumen %.1f Tonnen, %@, %d Prozent abgeschlossen, am %@", w.volume / 1000, setLabel, completion, date)
    case .run(let r):
