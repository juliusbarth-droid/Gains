...
final class GainsStore: ObservableObject {
  ...
  // 2026-05-14 (Audit-Loop 2): didSet-Hooks pingen den ActiveWorkoutPersister
  // bei jeder Mutation an. Persister debouncet intern (350ms), sodass
  // jeder ±-Tap auf Reps/Weight nicht 1:1 in einen UserDefaults-Write
  // mündet.
  ...
  // 2026-05-14 (Audit-Loop 3): Beim Cold-Start gefundener gespeicherter
  // Workout, der dem User per Banner zur Wiederherstellung angeboten wird.
  // Nicht gleich nach `loadPersistedData` als activeWorkout setzen — der
  // User entscheidet via „Fortsetzen / Verwerfen", was passieren soll.
  @Published var recoverableWorkout: PersistedWorkoutSession?

  // 2026-05-14 (Brand-Loop 5): Completion-Ritual-Trigger. Wird in
  // `finishWorkout()` gesetzt und vom Home-Screen als 3-Sek-Fullscreen-
  // Cover gerendert. Sobald die Ritual-Animation durch ist, setzt der
  // Cover den Wert zurück auf nil.
  @Published var pendingCompletionRitual: CompletedWorkoutSummary?
  ...
  @Published var workoutHistory: [CompletedWorkoutSummary] = [] {
    didSet { _cachedFourWeeks = nil; _cachedStrengthProgress = nil; _cachedYearActivity = nil }
  }
  // 2026-05-14 Bug-Fix: Cache wird auf nil gesetzt, sobald sich
  // `runHistory` ändert — verhindert, dass stale Slice-Daten geliefert
  // werden. (Property `cachedRunSlices` ist weiter unten in dieser
  // Klasse als private var deklariert.)
  @Published var runHistory: [CompletedRunSummary] = [] {
    didSet { cachedRunSlices = nil; _cachedFourWeeks = nil; _cachedYearlyRunStats = nil; _cachedYearActivity = nil; _cachedHeatmapTiles = nil }
  }
  // Strava-Erweiterung: Routen, Segmente, strukturierte Workouts.
  @Published var savedRoutes: [SavedRoute] = [] {
    // Heatmap-Cache invalidieren — savedRoutes fließen neben runHistory in
    // `routeHeatmapTiles` ein (siehe GainsStore+RunFeatures.swift).
    didSet { _cachedHeatmapTiles = nil }
  }
  ...
  @Published var savedWorkoutPlans: [WorkoutPlan] = WorkoutPlan.starterTemplates {
    didSet { _cachedWeeklySchedule = nil }
  }
  @Published var plannerSettings: WorkoutPlannerSettings = .default {
    didSet { invalidatePlannerCache() }   // invalidiert auch _cachedWeeklySchedule
  }
  /// 2026-05-30 — Session-Stack pro Wochentag für den vereinheitlichten
  /// Wochenplaner. Quelle der Wahrheit für Hybrid-Tage (Gym + Kardio am
  /// selben Tag). Wird beim ersten Laden aus dem Legacy-Plan migriert
  /// (siehe `migrateLegacyPlanToDaySessionsIfNeeded`), und über
  /// `syncLegacyMirror()` zurück in die alten Plan-Felder gespiegelt, damit
  /// Today-Tab & Vorschau konsistent bleiben. CRUD: siehe
  /// `GainsStore+WeekPlanner`.
  @Published var daySessions: [Weekday: [PlannedSession]] = [:]
  ...
  @Published var nutritionEntries: [NutritionEntry] = [] {
    didSet {
      _cachedTodayMacros = nil
      nutritionEntriesRevision &+= 1
    }
  }
  /// Monotone, billige Invalidierungsmarke für Nutrition-Views.
  /// SwiftUI-`onChange` auf dem kompletten `[NutritionEntry]` vergleicht bei
  /// jeder Mutation die ganze Historie. Ein Int-Vergleich reicht, weil jede
  /// Mutation durch `nutritionEntries.didSet` läuft.
  private(set) var nutritionEntriesRevision: Int = 0
  /// 2026-05-16 (Fertiger-Audit P0-5): Letzter via Tracker, Recipe-Quick-Track,
  /// Photo-Recognition oder Barcode geloggter Eintrag — die NutritionTrackerView
  /// rendert für diese ID einen kurzen Lime-Glow-Pulse auf der Row, der nach
  /// ~1.5 s verblasst. Vorher liefen Logs still ab und der User war unsicher,
  /// ob die Aktion gegriffen hat.
  @Published var lastLoggedNutritionEntryID: UUID?
  ...
  @Published var userAvatarData: Data? = nil {
    // Perf (2026-05-31): Decode-Cache entwerten, sobald sich das Bild ändert.
    didSet { _avatarImageDecoded = false }
  }
  ...
  // todayMacroTotals: Tages-Makros werden von 4 Properties (calories/protein/
  // carbs/fat) abgerufen. Ohne Cache: 4 separate O(n)-Walks durch nutritionEntries
  // pro Render-Pass. Mit Cache: 1 Walk pro Tag + Invalidierung via didSet.
  private var _cachedTodayMacros: (calories: Int, protein: Int, carbs: Int, fat: Int)?
  private var _todayMacrosCachedDate: Date = .distantPast

  // weeklyWorkoutSchedule ist das teuerste computed property in HomeView:
  // Es wird ~10× pro Render-Pass via `todayPlannedDay` aufgerufen. Cache
  // speichert das berechnete Array und wird gemeinsam mit dem Planner-Cache
  // invalidiert. Zusätzlich Datums-basierte Invalidierung (über Mitternacht).
  private var _cachedWeeklySchedule: [WorkoutDayPlan]? = nil
  private var _weeklyScheduleCachedDate: Date = .distantPast

  // MARK: - 4-Wochen-Plan-Cache
  // `nextFourWeeksSchedule` baut 28 Tages-Einträge + scannt workoutHistory
  // + runHistory für den completedDaySet. Ohne Cache: teurer O(n)-Pass auf
  // jede HomeView-Render-Runde (egal ob GPS, HF oder Nutrition changed).
  // Cache wird auf nil gesetzt, wenn:
  //   • workoutHistory / runHistory ändert sich (didSet oben)
  //   • plannerSettings / savedWorkoutPlans ändert sich (invalidatePlannerCache)
  //   • Datumswechsel (Mitternacht) → date-based check beim Lesen
  private var _cachedFourWeeks: [GymPlanPreviewWeek]?
  private var _cachedFourWeeksDate: Date = .distantPast

  // MARK: - exerciseStrengthProgress Cache
  // Sehr teures Property: flatMap über die komplette History (alle Exercises),
  // Dictionary-Grouping, sortedEntries.sorted pro Gruppe, plus String-Parsing
  // beim Schluss-Sort. Wird in GymStatsTab und ProgressView bei jedem Render
  // aufgerufen. Cache wird bei workoutHistory-Änderungen geleert.
  private var _cachedStrengthProgress: [ExerciseStrengthProgress]?

  // MARK: Heatmap-Tiles Cache
  //
  // `routeHeatmapTiles` (GainsStore+RunFeatures.swift) walkt ALLE GPS-Punkte
  // aus runHistory + savedRoutes — O(Summe aller Routenpunkte), wächst mit
  // jedem geloggten Lauf. RunRoutesView liest die Tiles 2× pro Render (Count +
  // Map). Ohne Cache lief der komplette Walk bei jeder Store-Mutation, solange
  // der Screen offen war. Invalidierung via runHistory.didSet + savedRoutes.didSet.
  // `internal` (kein private), damit die Extension in anderem File zugreifen kann.
  var _cachedHeatmapTiles: [RouteHeatmapTile]?

  // MARK: Year-Activity-Grid Cache
  //
  // Wird von ProgressView für die 365-Tage-Heatmap genutzt. Berechnung ist
  // O(n_workouts + n_runs + 365) — teuer genug um gecacht zu werden.
  // Invalidierung via workoutHistory.didSet und runHistory.didSet.

  struct YearActivityData {
    let weeks: [[Int?]]   // weeks[weekIdx][0…6] = Intensität (0–3) oder nil
    let totalActiveDays: Int
  }
  private var _cachedYearActivity: YearActivityData?
  private var _cachedYearActivityDate: Date = .distantPast

  var yearActivityData: YearActivityData {
    let calendar = Calendar(identifier: .gregorian)
    let today = calendar.startOfDay(for: Date())
    if let c = _cachedYearActivity, _cachedYearActivityDate == today { return c }
    let result = _computeYearActivityData(calendar: calendar, today: today)
    _cachedYearActivity = result
    _cachedYearActivityDate = today
    return result
  }

  private func _computeYearActivityData(calendar: Calendar, today: Date) -> YearActivityData {
    guard let startBoundary = calendar.date(byAdding: .day, value: -364, to: today) else {
      return YearActivityData(weeks: [], totalActiveDays: 0)
    }
    var counter: [Date: Int] = [:]
    for w in workoutHistory {
      let key = calendar.startOfDay(for: w.finishedAt)
      if key >= startBoundary { counter[key, default: 0] += 1 }
      else { break } // newest-first → früher Ausstieg
    }
    for r in runHistory {
      let key = calendar.startOfDay(for: r.finishedAt)
      if key >= startBoundary { counter[key, default: 0] += 1 }
      else { break } // newest-first → früher Ausstieg
    }
    let totalActiveDays = counter.keys.count
    let startWeekday = calendar.component(.weekday, from: startBoundary)
    let mondayOffset = (startWeekday + 5) % 7
    guard let firstMonday = calendar.date(byAdding: .day, value: -mondayOffset, to: startBoundary) else {
      return YearActivityData(weeks: [], totalActiveDays: totalActiveDays)
    }
    var weeks: [[Int?]] = []
    ...
    while cursor <= today {
      var column: [Int?] = []
      for _ in 0..<7 {
        if cursor < startBoundary || cursor > today {
          column.append(nil)
        } else {
          column.append(min(counter[cursor] ?? 0, 3))
        }
    ...
    return YearActivityData(weeks: weeks, totalActiveDays: totalActiveDays)
  }
  ...
  func invalidatePlannerCache() {
    ...
    _cachedWeeklySchedule = nil
    _cachedFourWeeks = nil
  }
  ...
  func scheduleSave(force: Bool = false) {
    ...
    _pendingSaveWork = nil
    if force {
      // Sofort speichern — kein Delay. Wichtig beim Onboarding-Finish
      // und finishWorkout(), damit der Folgeschritt auf persistierte
      // Daten aufbauen kann (z.B. hasCompletedOnboarding setzen).
      saveAll()
      // Fix 2026-05-16: ActiveWorkoutPersister hat einen eigenen 350ms-
      // Debounce-Task. Beim App-Backgrounding (force == true) könnte ein
      // frisch geloggter Satz noch im Debounce-Fenster liegen und bei
      // OS-Termination verloren gehen. Wir flushen daher sofort, wenn
      // ein Workout aktiv ist.
      if let workout = activeWorkout {
        let snapshot = PersistedWorkoutSession(
          workout: workout,
          restTimerEndsAt: activeRestTimerEndsAt,
          restDuration: activeRestDuration
        )
        ActiveWorkoutPersister.shared.flush(snapshot)
      }
      return
    }
  ...
  init() {
    ...
    recalculateStreak()
    // 2026-05-14 Audit-Loop 3: Recoverable-Workout aus dem Persister
    // einlesen. Wird vom Home-Screen als Banner angezeigt; setzt KEINE
    // activeWorkout sofort, der User entscheidet.
    recoverableWorkout = ActiveWorkoutPersister.shared.load()
  }

  // MARK: - Active-Workout Persistence (Audit-Loop 2, 2026-05-14)

  /// Aufgerufen aus den `didSet`-Hooks der `activeWorkout`/`activeRest…`-
  /// Properties. Beim Clear (activeWorkout == nil) wird der Persister
  /// explizit geleert; sonst wird ein neuer Snapshot debounced geschrieben.
  private func persistActiveWorkout() {
    guard let workout = activeWorkout else {
      ActiveWorkoutPersister.shared.clear()
      return
    }
    let snapshot = PersistedWorkoutSession(
      workout: workout,
    ...
    ActiveWorkoutPersister.shared.scheduleSave(snapshot)
  }
  ...
  /// Discard-Pfad: User möchte den gespeicherten Workout nicht zurück.
  func discardRecoverableWorkout() {
    ActiveWorkoutPersister.shared.clear()
    recoverableWorkout = nil
  }
  ...
  private func loadPersistedData(today: Date) {
    ...
    if let plans = ud.decodedLoad([WorkoutPlan].self, forKey: PersistenceKey.savedWorkoutPlans) {
      ...
      // Append-only Migration: neue evidenzbasierte Starter-Vorlagen
      // (z. B. PPL B-Tage, Bro-Split-Tage) zu bestehenden Bibliotheken
      // hinzufügen, ohne Custom-Plans oder eigene Anpassungen anzu-
      // tasten. Match per Titel — wenn ein Titel schon existiert
      // (auch wenn vom User bearbeitet), wird er übersprungen.
      mergeMissingStarterTemplates()
    }
    if let settings = ud.decodedLoad(WorkoutPlannerSettings.self, forKey: PersistenceKey.plannerSettings) {
      plannerSettings = settings
    }
    // 2026-05-30 — Session-Stack laden ([String: [PlannedSession]] mit
    // Weekday.rawValue als Key, analog zur Codable-Konvention von
    // WorkoutPlannerSettings). Danach ggf. aus dem Legacy-Plan migrieren,
    // falls noch nichts gespeichert wurde.
    if let rawSessions = ud.decodedLoad([String: [PlannedSession]].self, forKey: PersistenceKey.daySessions) {
      daySessions = Dictionary(uniqueKeysWithValues: rawSessions.compactMap { key, value -> (Weekday, [PlannedSession])? in
        guard let raw = Int(key), let day = Weekday(rawValue: raw) else { return nil }
        return (day, value)
      })
    }
    migrateLegacyPlanToDaySessionsIfNeeded()
  ...
  func saveAll(completion: (() -> Void)? = nil) {
    ...
    let _daySessions           = self.daySessions
    ...
    DispatchQueue.global(qos: .utility).async {
      ...
      // Session-Stack als [String: [PlannedSession]] (Weekday.rawValue-Key).
      let rawDaySessions = Dictionary(uniqueKeysWithValues: _daySessions.map {
        (String($0.key.rawValue), $0.value)
      })
      ud.encodedSave(rawDaySessions,       forKey: PersistenceKey.daySessions)
  ...
  // MARK: - Reset (Release-tauglich)
  //
  // 2026-05-16 (Release-Audit P0): aus dem `#if DEBUG`-Block herausgehoben,
  // weil ProfileView.optionsCard die Funktion auch im Release-Build aufruft
  // (DSGVO/App-Store-Pflicht: jede App mit User-Daten braucht eine sichtbare
  // Lösch-Option im UI). Vorher cascadierten daraus drei `dynamicMember`-
  // Errors in ProfileView, weil die Funktion im Non-DEBUG-Build schlicht nicht
  // existierte.
  ...
  var weeklyWorkoutSchedule: [WorkoutDayPlan] {
    let today = Calendar.current.startOfDay(for: Date())
    if let cached = _cachedWeeklySchedule, _weeklyScheduleCachedDate == today {
      return cached
    }
    let result = _computeWeeklyWorkoutSchedule()
    _cachedWeeklySchedule = result
    _weeklyScheduleCachedDate = today
    return result
  }

  private func _computeWeeklyWorkoutSchedule() -> [WorkoutDayPlan] {
      ...
      if scheduledWorkoutDays.contains(day) {
        ...
        if kind.isRun {
          ...
          let focus: String = {
            ...
            return "\(String(format: "%.1f km", template.targetDistanceKm)) · \(template.targetDurationMinutes) Min · \(template.targetPaceLabel)"
          }()
  ...
  //
  // 2026-05-14 (Perf-Loop): Alle Slices wurden als separate computed properties
  // implementiert, was bei jedem Body-Render zu N×O(n)-Filtern führte:
  //   • runOnlyHistory     → filter(runHistory)
  //   • bikeOnlyHistory    → filter(runHistory)
  //   • recentRunOnly      → filter(runOnlyHistory)   ← second pass
  //   • recentBike         → filter(bikeOnlyHistory)  ← second pass
  //   • weeklyRunDistance  → recentRunHistory.reduce  ← recentRunHistory = third pass
  //   • averageRunPace     → recentRunOnly × 3 (isEmpty + 2 reduce)
  // Jetzt: EIN Partition-Pass durch `runHistory` pro Cache-Miss. Cache wird
  // via didSet auf `runHistory` invalidiert.

  private struct RunHistorySlices {
    // Alle Run-only Sessions (modality == .run), neueste zuerst.
    let runOnly: [CompletedRunSummary]
    // Alle Bike Sessions (outdoor + indoor), neueste zuerst.
    let bikeOnly: [CompletedRunSummary]
    // Letzte 7 Tage, run-only.
    let recentRunOnly: [CompletedRunSummary]
    // Letzte 7 Tage, bike-only.
    let recentBike: [CompletedRunSummary]
    // Letzte 7 Tage, alle Modalitäten (Cardio-Total).
    let recentAll: [CompletedRunSummary]
    // Letzte 30 Tage, alle Modalitäten.
    let last30DaysAll: [CompletedRunSummary]
  }

  // Cache — nil nach jedem runHistory-didSet oder wenn das 7-Tage-Fenster
  // abgelaufen ist. Wir speichern zusätzlich das Datum der Berechnung, damit
  // ein über-Mitternacht-Fehler (recentAll wird schmal) den Cache invalididert.
  //
  // 2026-05-14 Bug-Fix: Vorher waren diese Properties mit Underscore-
  // Präfix deklariert. Das hat in bestimmten Xcode-Versionen zu
  // Scope-Errors im didSet von `runHistory` geführt — Swift verwechselt
  // underscore-prefixed Properties mit dem Synthesized-Storage von
  // @Published-Wrappers. Jetzt ohne Underscore, eindeutig.
  private var cachedRunSlices: RunHistorySlices?
  private var runSlicesCachedDate: Date = .distantPast

  private var runHistorySlices: RunHistorySlices {
    let cal = Calendar.current
    let now = Date()
    let today = cal.startOfDay(for: now)
    if let cached = cachedRunSlices, runSlicesCachedDate == today {
      return cached
    }
    let cutoff7  = cal.date(byAdding: .day, value:  -7, to: now) ?? Date.distantPast
    let cutoff30 = cal.date(byAdding: .day, value: -30, to: now) ?? Date.distantPast
    var runOnly:      [CompletedRunSummary] = []
    var bikeOnly:     [CompletedRunSummary] = []
    var recentRun:    [CompletedRunSummary] = []
    var recentBike:   [CompletedRunSummary] = []
    var recentAll:    [CompletedRunSummary] = []
    var last30All:    [CompletedRunSummary] = []
    for s in runHistory {
      let isRecent7  = s.finishedAt >= cutoff7
      let isRecent30 = s.finishedAt >= cutoff30
      if s.modality == .run {
        runOnly.append(s)
        if isRecent7  { recentRun.append(s); recentAll.append(s) }
        if isRecent30 { last30All.append(s) }
      } else if s.modality.isCycling {
        bikeOnly.append(s)
        if isRecent7  { recentBike.append(s); recentAll.append(s) }
        if isRecent30 { last30All.append(s) }
      } else {
        if isRecent7  { recentAll.append(s) }
        if isRecent30 { last30All.append(s) }
      }
    }
    let slices = RunHistorySlices(
      runOnly: runOnly, bikeOnly: bikeOnly,
      recentRunOnly: recentRun, recentBike: recentBike,
      recentAll: recentAll, last30DaysAll: last30All
    )
    cachedRunSlices = slices
    runSlicesCachedDate = today
    return slices
  }

  /// Run-only Slice der `runHistory` (kein Bike).
  private var runOnlyHistory: [CompletedRunSummary] { runHistorySlices.runOnly }

  /// Bike-only Slice der `runHistory` (Outdoor + Indoor).
  private var bikeOnlyHistory: [CompletedRunSummary] { runHistorySlices.bikeOnly }

  /// Letzte 7 Tage, run-only.
  private var recentRunOnlyHistory: [CompletedRunSummary] { runHistorySlices.recentRunOnly }

  /// Letzte 7 Tage, bike-only.
  private var recentBikeHistory: [CompletedRunSummary] { runHistorySlices.recentBike }
  ...
  /// Single-Pass: vorher 3× iterate auf recentRunOnlyHistory (isEmpty + 2 reduce).
  var averageRunPaceSeconds: Int {
    let recent = runHistorySlices.recentRunOnly
  ...
  /// Single-Pass: vorher 3× iterate auf recentBikeHistory.
  var averageBikeSpeedKmh: Double {
    let recent = runHistorySlices.recentBike
    guard !recent.isEmpty else { return 0 }
    var totalDistance = 0.0
    var totalSeconds  = 0
    for s in recent {
  ...
  var bestRunPaceSeconds: Int {
    runHistorySlices.runOnly.map(\.averagePaceSeconds).min() ?? 0
  }
  ...
  /// Ein Wochen-Bucket für den 8-Wochen-Trend (Sparkline im Kardio-Hub).
  struct WeeklyDistanceBucket: Identifiable {
    let id = UUID()
    /// Kurz-Label, z. B. "KW22".
    let weekLabel: String
    let km: Double
    /// True für die laufende Woche (rechtester Balken, hervorgehoben).
    let isCurrent: Bool
  }
  ...
  var weeklyRunsByDay: [DailyRunDistance] {
    ...
    // Single-pass: baue [startOfDay → km]-Dict, dann 7 Einträge abbilden.
    // Vorher: 7 × O(n_runs) filter-passes = O(7n). Jetzt: O(n) + O(7).
    let earliest = calendar.date(byAdding: .day, value: -6, to: today) ?? today
    var kmByDay: [Date: Double] = [:]
    for run in runHistory {
      let day = calendar.startOfDay(for: run.finishedAt)
      guard day >= earliest else { break } // Historie ist newest-first
      kmByDay[day, default: 0] += run.distanceKm
    }
    return (0..<7).reversed().map { daysAgo -> DailyRunDistance in
      let date = calendar.date(byAdding: .day, value: -daysAgo, to: today) ?? today
      let weekdayIndex = calendar.component(.weekday, from: date) - 1
      let label = String(symbols[weekdayIndex].prefix(2))
      return DailyRunDistance(dayLabel: label, km: kmByDay[date] ?? 0, isToday: daysAgo == 0)
    }
  }

  /// Wochen-Distanz-Trend der letzten `weeks` Wochen (älteste → laufende),
  /// gefiltert nach Modalität. Lauf-Modus zählt nur `.run`, Bike-Modus alle
  /// Cycling-Sessions (Outdoor + Indoor). Treibt die Sparkline im Hub.
  /// Single-Pass über `runHistory`: ein `[Wochenstart → km]`-Dict, dann
  /// `weeks` Einträge abbilden — analog `weeklyRunsByDay`.
  func weeklyDistanceTrend(modality: CardioModality, weeks: Int = 8) -> [WeeklyDistanceBucket] {
    let calendar = Calendar.current
    guard let currentWeekStart = calendar.dateInterval(of: .weekOfYear, for: Date())?.start
    else { return [] }
    let earliest = calendar.date(byAdding: .weekOfYear, value: -(weeks - 1), to: currentWeekStart)
      ?? currentWeekStart

    var kmByWeek: [Date: Double] = [:]
    for run in runHistory {
      let matches = modality.isCycling ? run.modality.isCycling : (run.modality == .run)
      guard matches else { continue }
      guard let weekStart = calendar.dateInterval(of: .weekOfYear, for: run.finishedAt)?.start
      else { continue }
      guard weekStart >= earliest else { break } // Historie ist newest-first
      kmByWeek[weekStart, default: 0] += run.distanceKm
    }

    return (0..<weeks).reversed().map { weeksAgo -> WeeklyDistanceBucket in
      ...
      return WeeklyDistanceBucket(
        weekLabel: "KW\(weekNumber)",
        km: kmByWeek[weekStart] ?? 0,
        isCurrent: weeksAgo == 0
      )
    }
  }

  /// Gemeinsames Jahres-Cutoff — wird einmalig pro Zugriff berechnet,
  /// nicht für jede der 4 folgenden Properties neu.
  private var startOfCurrentYear: Date {
    Calendar.current.date(
      from: Calendar.current.dateComponents([.year], from: Date())) ?? Date.distantPast
  }

  // MARK: Jahres-Stats Cache
  //
  // Vier Properties (distanceKm, count, elevationGain, durationMinutes) haben
  // alle dasselbe Filter-Prädikat (finishedAt >= Jahresstart). Statt 4 × O(n)
  // filter+reduce wird hier einmal O(n) durchgelaufen und das Ergebnis
  // gecacht. Invalidierung läuft über runHistory.didSet (s.o.).

  private struct YearlyRunStatsCache {
  ...
  private var _cachedYearlyRunStats: YearlyRunStatsCache?

  private var yearlyRunStatsCache: YearlyRunStatsCache {
    let currentYear = Calendar.current.component(.year, from: Date())
    if let c = _cachedYearlyRunStats, c.year == currentYear { return c }
    let cutoff = startOfCurrentYear
    var cache = YearlyRunStatsCache(year: currentYear)
    for run in runHistorySlices.runOnly {
      guard run.finishedAt >= cutoff else { continue }
      cache.distanceKm      += run.distanceKm
      cache.count           += 1
      cache.durationMinutes += run.durationMinutes
      cache.elevationGainM  += run.elevationGain
    }
    // Outdoor-Bike trägt Höhenmeter bei, aber keine Lauf-Distanz/-Dauer.
    for run in runHistorySlices.bikeOnly {
      guard !run.modality.isIndoor, run.finishedAt >= cutoff else { continue }
      cache.elevationGainM += run.elevationGain
    }
    _cachedYearlyRunStats = cache
    return cache
  }

  var yearlyRunDistanceKm: Double    { yearlyRunStatsCache.distanceKm }
  var yearlyRunCount: Int            { yearlyRunStatsCache.count }
  var yearlyElevationGain: Int       { yearlyRunStatsCache.elevationGainM }
  var yearlyRunDurationMinutes: Int  { yearlyRunStatsCache.durationMinutes }
  ...
  var paceZones: [PaceZoneEntry] {
    ...
    // Single-pass statt 4 × filter-pass über dasselbe Array.
    var easy = 0, moderate = 0, tempo = 0, hard = 0
    for pace in paces {
      if pace >= 420 { easy += 1 }
      else if pace >= 330 { moderate += 1 }
      else if pace >= 270 { tempo += 1 }
      else { hard += 1 }
    }
    let total = Double(paces.count)
    let entries: [(String, String, Int)] = [
      ("Easy",    ">7:00 /km",  easy),
      ("Moderat", "5:30–7:00",  moderate),
      ("Tempo",   "4:30–5:30",  tempo),
      ("Hart",    "<4:30 /km",  hard),
    ]
    return entries.filter { $0.2 > 0 }.map {
      PaceZoneEntry(label: $0.0, description: $0.1, fraction: Double($0.2) / total)
    }
  }
  ...
  var distancePRs: [RunPersonalBest] {
    ...
    // Single-pass: prüfe jeden Run gegen alle 4 Zieldistanzen gleichzeitig,
    // statt 4 × separater filter+min über runOnlyHistory (O(4n) → O(n)).
    // `bestTime[i]` hält (normalisierteMinuten, run) für Distanz targets[i].
    var bestTime: [(Double, CompletedRunSummary)?] = Array(repeating: nil, count: targets.count)
    for run in runOnlyHistory {
      for (i, target) in targets.enumerated() {
        guard run.distanceKm >= target.minKm else { continue }
        let t = Double(run.durationMinutes) * (target.exactKm / run.distanceKm)
        if let existing = bestTime[i] {
          if t < existing.0 { bestTime[i] = (t, run) }
        } else {
          bestTime[i] = (t, run)
        }
      }
    }
    return zip(targets, bestTime).compactMap { (target, best) -> RunPersonalBest? in
      guard let (t, run) = best else { return nil }
      let estimatedMinutes = Int(t)
      let h = estimatedMinutes / 60
      let m = estimatedMinutes % 60
      let timeString = h > 0 ? "\(h)h \(m)min" : "\(m) min"
      return RunPersonalBest(title: target.title, value: timeString, context: run.routeName)
    }
  }
  ...
  var yearlyBikeDistanceKm: Double {
    let cutoff = startOfCurrentYear
    return runHistorySlices.bikeOnly
      .filter { $0.finishedAt >= cutoff }.reduce(0) { $0 + $1.distanceKm }
  }

  /// YTD Bike-Anzahl.
  var yearlyBikeCount: Int {
    let cutoff = startOfCurrentYear
    return runHistorySlices.bikeOnly.filter { $0.finishedAt >= cutoff }.count
  }
  ...
  var yearlyBikeDurationMinutes: Int {
    let cutoff = startOfCurrentYear
    return runHistorySlices.bikeOnly
      .filter { $0.finishedAt >= cutoff }.reduce(0) { $0 + $1.durationMinutes }
  }
  ...
  var yearlyBikeElevationGain: Int {
    let cutoff = startOfCurrentYear
    return runHistorySlices.bikeOnly
      .filter { $0.finishedAt >= cutoff && !$0.modality.isIndoor }
  ...
  private var todayMacroTotals: (calories: Int, protein: Int, carbs: Int, fat: Int) {
    let today = Calendar.current.startOfDay(for: Date())
    if let cached = _cachedTodayMacros, _todayMacrosCachedDate == today {
      return cached
    }
    var result = (calories: 0, protein: 0, carbs: 0, fat: 0)
    let cal = Calendar.current
    for e in nutritionEntries where cal.isDate(e.loggedAt, inSameDayAs: today) {
      result.calories += e.calories
      result.protein  += e.protein
      result.carbs    += e.carbs
      result.fat      += e.fat
    }
    _cachedTodayMacros    = result
    _todayMacrosCachedDate = today
    return result
  }
  ...
  func logRecipe(_ recipe: Recipe) {
    ...
    lastLoggedNutritionEntryID = entry.id
  ...
  var nutritionStreakDays: Int {
    // Vorher: O(n × streak) — für jeden Streak-Tag ein vollständiger
    // `nutritionEntries.contains` Scan. Jetzt: O(n) einmaliger Pass
    // → aktiveDays-Set, dann O(streak) Rückwärts-Lookup in O(1).
    let calendar = Calendar.current
    let activeDays = Set(nutritionEntries.map {
    ...
    while true {
      let date = calendar.date(byAdding: .day, value: -daysBack, to: Date()) ?? Date()
      let startOfDay = calendar.startOfDay(for: date)
      if activeDays.contains(startOfDay) {
        streak += 1
        daysBack += 1
      } else {
        break
  ...
  func dailyCalories(lastDays days: Int) -> [Int] {
    // Vorher: N × O(n) filter+reduce. Jetzt: O(n) Single-Pass → Dict,
    // dann O(days) Lookups.
    let calendar = Calendar.current
    let today = calendar.startOfDay(for: Date())
    let n = max(1, days)
    guard let earliest = calendar.date(byAdding: .day, value: -(n - 1), to: today) else {
      return Array(repeating: 0, count: n)
    }
    var kcalByDay: [Date: Int] = [:]
    for entry in nutritionEntries {
      let day = calendar.startOfDay(for: entry.loggedAt)
      guard day >= earliest else { continue }
      kcalByDay[day, default: 0] += entry.calories
    }
    return (0..<n).reversed().map { offset -> Int in
      let day = calendar.date(byAdding: .day, value: -offset, to: today) ?? today
      return kcalByDay[day] ?? 0
    }
  }
  ...
  func proteinHitRate(lastDays days: Int) -> Double {
    ...
    let threshold = Double(target) * 0.9
    guard let earliest = calendar.date(byAdding: .day, value: -(total - 1), to: today) else {
      return 0
    }
    var proteinByDay: [Date: Int] = [:]
    for entry in nutritionEntries {
      let day = calendar.startOfDay(for: entry.loggedAt)
      guard day >= earliest else { continue }
    ...
    for offset in 0..<total {
      let day = calendar.date(byAdding: .day, value: -offset, to: today) ?? today
      if Double(proteinByDay[day] ?? 0) >= threshold { hits += 1 }
    }
  ...
  // Single-pass over the last-7-days slice — count + volume in one go.
  private var weeklyWorkoutStats: (count: Int, volumeTons: Double) {
    let cutoff = Calendar.current.date(byAdding: .day, value: -7, to: Date()) ?? Date.distantPast
    var count = 0
    var volumeKg = 0.0
    for w in workoutHistory {
      guard w.finishedAt >= cutoff else { break }  // newest-first → early exit
      count += 1
      volumeKg += w.volume
    }
    return (count, volumeKg / 1000)
  }

  // Bug-Fix: Vorher zählte weeklySessionsCompleted nur Kraft-Sessions aus
  // workoutHistory — abgeschlossene Läufe flossen nicht ein. Bei Hybrid-
  // Plänen (z.B. 3× Kraft + 2× Lauf) blieb der Zähler dauerhaft unter dem
  // Wochenziel. Jetzt werden beide Historien der letzten 7 Tage summiert
  // — nutzt die öffentliche `weeklyRunCount` oben (cached slices).
  // 2026-05-15: Duplicate `private var weeklyRunCount` entfernt — der
  // Linter hatte oben bereits eine öffentliche Version eingeführt, die
  // hier dann zweimal deklariert wurde (Build-Error „Invalid redeclaration").
  var weeklySessionsCompleted: Int { weeklyWorkoutStats.count + weeklyRunCount }
  var weeklyVolumeTons: Double { weeklyWorkoutStats.volumeTons }
  ...
  var exerciseStrengthProgress: [ExerciseStrengthProgress] {
    if let cached = _cachedStrengthProgress { return cached }
    let result = _computeExerciseStrengthProgress()
    _cachedStrengthProgress = result
    return result
  }

  private func _computeExerciseStrengthProgress() -> [ExerciseStrengthProgress] {
    // Single-Pass über History: statt flatMap→groupBy→sort pro Gruppe eine
    // manuelle Akkumulation mit (earliest, latest) pro Übungsname.
    // Erspart: N×sort(pro Gruppe) + String-Parsing beim Schluss-Sort.
    var earliest: [String: (Date, CompletedExercisePerformance)] = [:]
    var latest:   [String: (Date, CompletedExercisePerformance)] = [:]

    for workout in workoutHistory {
      let date = workout.finishedAt
      for ex in workout.exercises where ex.topWeight > 0 {
        let name = ex.name
        if let e = earliest[name] {
          if date < e.0 { earliest[name] = (date, ex) }
        } else {
          earliest[name] = (date, ex)
        }
        if let l = latest[name] {
          if date > l.0 { latest[name] = (date, ex) }
        } else {
          latest[name] = (date, ex)
        }
      }
    ...
    var result: [ExerciseStrengthProgress] = []
    result.reserveCapacity(min(latest.count, 4))
    for (name, (_, latestEx)) in latest {
      let baselineEx = earliest[name]?.1 ?? latestEx
      let delta = latestEx.topWeight - baselineEx.topWeight
      result.append(ExerciseStrengthProgress(
        exerciseName: name,
        currentValue: String(format: "%.1f kg", latestEx.topWeight),
        deltaLabel: strengthDeltaLabel(delta, baseline: baselineEx.topWeight),
        subtitle: "\(latestEx.completedSets) Sätze · \(Int(latestEx.totalVolume)) kg Volumen",
        _topWeightValue: latestEx.topWeight
      ))
    }
    // Sortierung direkt auf Double statt String-Parsing.
    result.sort { $0._topWeightValue > $1._topWeightValue }
  ...
  var currentBloodPanelSummary: [HealthMetric] {
    let weightDelta = max(startWeight - currentWeight, 0)
    let wsc = weeklySessionsCompleted  // single recentWorkoutHistory traversal
    let ldlValue = max(88, Int(112 - weightDelta * 2.1))
    let hba1cValue = max(4.7, 5.2 - (weightDelta * 0.04))
    let crpValue = max(
      0.6,
      1.1 - (Double(wsc) * 0.04) - (Double(connectedTrackerIDs.count) * 0.03))
  ...
  var coachHeadline: String {
    if let activeWorkout {
      let s = activeWorkout.stats
      return "Coach sagt: \(s.completedSets) von \(s.totalSets) Sätzen sind erledigt"
    }
  ...
  var calendarWeekTitle: String {
    ...
    let start = startOfWeek(
      for: cal.date(byAdding: .weekOfYear, value: calendarWeekOffset, to: Date())
        ?? Date())
  ...
  func syncActiveRunGPS(
    ...
    // Batch-Mutation: alle 5 Felder auf einmal setzen → eine einzige
    // objectWillChange-Benachrichtigung statt 5 × (Optional-get + set).
    // Vorher wurden 5 separate `activeRun?.x = y`-Zuweisungen ausgeführt,
    // jede triggert den @Published-Setter und schickt eine Notification
    // an alle Subscriber (HomeView, WorkoutTrackerView, etc.).
    guard var run = activeRun else { return }
    run.distanceKm = rounded(max(distanceKm, 0))
    run.durationMinutes = max(durationMinutes, 0)
    run.elevationGain = max(elevationGain, 0)
    run.routeCoordinates = routeCoordinates
    run.splits = splits
    activeRun = run
  }
  ...
  func addRunSplit(
    ...
    // Batch-Mutation: alle Felder auf einem lokalen Struct-Copy setzen,
    // dann einmalig zuweisen → eine objectWillChange-Notification statt 5.
    guard var run = activeRun, !run.isPaused else { return }
    run.distanceKm = rounded(max(0, run.distanceKm + distance))
    run.durationMinutes = max(0, run.durationMinutes + durationMinutes)
    run.elevationGain = max(0, run.elevationGain + elevation)
    if heartRate != 0 {
      run.currentHeartRate = min(max(run.currentHeartRate + heartRate, 40), 220)
    } else {
      run.currentHeartRate = min(max(run.currentHeartRate + Int.random(in: -2...3), 108), 182)
    }
    let split = RunSplit(
      id: UUID(),
      index: run.splits.count + 1,
      distanceKm: distance,
  ...
  func finishRun(
    ...
    HealthKitManager.shared.saveRunWorkout(
      ...
      isIndoor: summary.modality.isIndoor
    )
  ...
  /// Append-only Migration für neue Starter-Vorlagen. Beim App-Start
  /// vergleichen wir die aktuelle Library `WorkoutPlan.starterTemplates`
  /// mit den persistierten `savedWorkoutPlans`. Jede neue Vorlage (matched
  /// per Titel) wird hinten angehängt — bestehende Pläne, Custom-Workouts
  /// und Tageszuweisungen bleiben unberührt.
  ///
  /// Warum nur Append, nicht Replace: Tageszuweisungen referenzieren
  /// `plan.id`. Ein Replace würde IDs neu vergeben und damit alle
  /// `dayAssignments[…] = oldID` brechen.
  func mergeMissingStarterTemplates() {
    let existingTitles = Set(savedWorkoutPlans.map { $0.title })
    let missing = WorkoutPlan.starterTemplates.filter {
      !existingTitles.contains($0.title)
    }
    guard !missing.isEmpty else { return }
    savedWorkoutPlans.append(contentsOf: missing)
    scheduleSave()
  }
  ...
  func finishWorkout() {
    ...
    let ws = workout.stats  // single-pass — statt 3× Forwarder-Aufrufe
    guard ws.completedSets > 0 else {
      discardWorkout()
      return
    }
    ...
    activeWorkout = nil
    // 2026-05-14 (Brand-Loop 5): Trigger das Completion-Ritual. Home-View
    // hängt einen fullScreenCover an `pendingCompletionRitual` an und
    // zeigt den 3-Sek-Ritual-Screen.
    pendingCompletionRitual = summary
  ...
  func updateSet(exerciseID: UUID, setID: UUID, reps: Int? = nil, weight: Double? = nil) {
    // Batch-Mutation: lokale Kopie, alle Felder ändern, einmal zurückschreiben.
    // Verhindert 2× objectWillChange wenn beide Parameter gesetzt sind.
    guard var session = activeWorkout,
  ...
  func createCommunityPost(from action: CommunityComposerAction) {
    ...
    case .workout:
      ...
      if let workout {
        workoutMetrics = [
          CommunityMetric(label: "Volumen", value: String(format: "%.1f t", workout.volume / 1000)),
  ...
  func loadCommunityContacts() {
    ...
    // Fix 2026-05-16: `enumerateContacts` ist synchron und blockiert bei
    // vielen Kontakten den MainThread → UI-Freeze. Jetzt auf Background-Queue
    // ausgelagert; Ergebnis wird via DispatchQueue.main.async zurückgeschrieben.
  ...
  ///
  /// Perf (2026-05-31): Das Ergebnis wird gecacht, statt den JPEG-Blob bei
  /// jedem Body-Pass (HomeView-Greeting läuft ≥ alle 60 s) neu zu dekodieren.
  /// Invalidierung läuft über den `userAvatarData.didSet` oben.
  private var _cachedAvatarImage: UIImage?
  private var _avatarImageDecoded = false
  var userAvatarImage: UIImage? {
    if _avatarImageDecoded { return _cachedAvatarImage }
    _avatarImageDecoded = true
    guard let data = userAvatarData, !data.isEmpty else {
      _cachedAvatarImage = nil
      return nil
    }
    _cachedAvatarImage = UIImage(data: data)
  ...
  /// 2026-05-29 (Light-Mode-Pass): Direkter Setter für die Appearance-
  /// Picker-Row im Profil. Persistiert sofort, damit das System-/Hell-/
  /// Dunkel-Switching über App-Starts hinweg stabil bleibt.
  func setAppearanceMode(_ mode: GainsAppearanceMode) {
  ...
  // recentRunHistory (alle Modalitäten, letzte 7 Tage) → aus dem Cache.
  private var recentRunHistory: [CompletedRunSummary] { runHistorySlices.recentAll }
  ...
  /// Cache-invalidiert bei: workoutHistory, runHistory, plannerSettings, savedWorkoutPlans.
  var nextFourWeeksSchedule: [GymPlanPreviewWeek] {
    let calendar = Calendar.current
    let today = calendar.startOfDay(for: Date())
    if let cached = _cachedFourWeeks, _cachedFourWeeksDate == today {
      return cached
    }
    let result = _computeNextFourWeeksSchedule()
    _cachedFourWeeks = result
    _cachedFourWeeksDate = today
    return result
  }

  private func _computeNextFourWeeksSchedule() -> [GymPlanPreviewWeek] {
    let calendar = Calendar.current
  ...
  }  // end _computeNextFourWeeksSchedule
...
// MARK: - GainsStore + WeekPlanner
//
// 2026-05-30 — Logik-Schicht für den vereinheitlichten Wochenplaner auf Basis
// von `daySessions` (Session-Stack pro Wochentag). Migration aus dem
// Legacy-Plan, CRUD, Legacy-Spiegel, Done-Status pro Session und der ruhige
// Wochen-Check (`weeklyLoadCheck`).

extension GainsStore {

  // MARK: Kalender / Datum

  /// ISO-Kalender mit Montag als Wochenstart — passend zur deutschen Konvention.
  private var weekPlannerCalendar: Calendar {
    var cal = Calendar(identifier: .iso8601)
    cal.firstWeekday = 2
    return cal
  }

  /// Konkretes Datum für einen Wochentag in der aktuellen Woche.
  func currentWeekDate(for weekday: Weekday) -> Date {
    let cal = weekPlannerCalendar
    let today = Date()
    guard
      let weekStart = cal.date(from: cal.dateComponents([.yearForWeekOfYear, .weekOfYear], from: today)),
      let date = cal.date(byAdding: .day, value: weekday.mondayOffset, to: weekStart)
  ...
  /// Geordneter Session-Stack eines Wochentags.
  func sessions(for weekday: Weekday) -> [PlannedSession] {
    daySessions[weekday] ?? []
  }

  /// Workout-Vorlage hinter einer Gym-Session (falls zugewiesen).
  func workoutPlan(for session: PlannedSession) -> WorkoutPlan? {
    guard let id = session.workoutPlanID else { return nil }
    return savedWorkoutPlans.first(where: { $0.id == id })
  }

  /// Stage-1-Heuristik: eine Session gilt als erledigt, wenn an ihrem Datum
  /// eine passende Aktivität in der History liegt (Gym → workoutHistory,
  /// Kardio → runHistory).
  func isSessionCompleted(_ session: PlannedSession, on date: Date) -> Bool {
    let cal = Calendar.current
    if session.isGym {
      return workoutHistory.contains { cal.isDate($0.finishedAt, inSameDayAs: date) }
    }
    if session.isCardio {
      return runHistory.contains { cal.isDate($0.finishedAt, inSameDayAs: date) }
    }
    return false
  }

  /// Anzahl aller geplanten Sessions in der aktuellen Woche.
  var weekPlannerTotalSessions: Int {
    Weekday.allCases.reduce(0) { $0 + (daySessions[$1]?.count ?? 0) }
  }

  /// Anzahl der in dieser Woche bereits erledigten Sessions.
  var weekPlannerDoneSessions: Int {
    var done = 0
    for day in Weekday.allCases {
      let date = currentWeekDate(for: day)
      for session in daySessions[day] ?? [] where isSessionCompleted(session, on: date) {
        done += 1
      }
    }
    return done
  ...
  func addSession(_ session: PlannedSession, to weekday: Weekday) {
    daySessions[weekday, default: []].append(session)
    afterSessionMutation()
  }

  func removeSession(_ id: UUID, from weekday: Weekday) {
    daySessions[weekday]?.removeAll { $0.id == id }
    if daySessions[weekday]?.isEmpty == true { daySessions[weekday] = nil }
    afterSessionMutation()
  }

  func updateSession(_ session: PlannedSession, on weekday: Weekday) {
    guard let idx = daySessions[weekday]?.firstIndex(where: { $0.id == session.id }) else { return }
    daySessions[weekday]?[idx] = session
    afterSessionMutation()
  }

  /// Eine konkrete Session auf einen anderen Wochentag verschieben (ans Ende
  /// des Ziel-Stacks). Verallgemeinert das alte „Tag tauschen".
  func moveSession(_ id: UUID, from source: Weekday, to target: Weekday) {
    guard source != target,
          let idx = daySessions[source]?.firstIndex(where: { $0.id == id }),
          let session = daySessions[source]?.remove(at: idx) else { return }
    if daySessions[source]?.isEmpty == true { daySessions[source] = nil }
    daySessions[target, default: []].append(session)
    afterSessionMutation()
  }

  /// Umsortieren innerhalb eines Tages (Drag-Reorder).
  func reorderSessions(in weekday: Weekday, fromOffsets: IndexSet, toOffset: Int) {
    guard daySessions[weekday] != nil else { return }
    daySessions[weekday]?.move(fromOffsets: fromOffsets, toOffset: toOffset)
    afterSessionMutation()
  }

  /// Kompletten Stack eines Tages setzen.
  func setSessions(_ sessions: [PlannedSession], for weekday: Weekday) {
    daySessions[weekday] = sessions.isEmpty ? nil : sessions
    afterSessionMutation()
  }

  private func afterSessionMutation() {
    syncLegacyMirror()
    invalidatePlannerCache()
    scheduleSave()
  }

  // MARK: Migration & Legacy-Spiegel

  /// Baut `daySessions` einmalig aus dem bestehenden Plan auf. Idempotent.
  func migrateLegacyPlanToDaySessionsIfNeeded() {
    guard daySessions.isEmpty else { return }
    let kinds = plannedSessionKinds
    var built: [Weekday: [PlannedSession]] = [:]
    for day in Weekday.allCases {
      guard let kind = kinds[day] else { continue }
      if kind == .strength {
        built[day] = [PlannedSession(kind: .strength, workoutPlanID: plannerSettings.dayAssignments[day])]
      } else if kind.isRun {
        built[day] = [PlannedSession(kind: kind)]
      }
    }
    daySessions = built
  }

  /// Spiegelt die jeweils erste Session pro Tag zurück in die Legacy-Plan-Felder,
  /// damit Today-Tab, 4-Wochen-Vorschau und Coach konsistent bleiben.
  func syncLegacyMirror() {
    var prefs = plannerSettings.dayPreferences
    var kinds: [Weekday: PlannedSessionKind] = [:]
    var assigns = plannerSettings.dayAssignments

    for day in Weekday.allCases {
      let stack = daySessions[day] ?? []
      if let first = stack.first {
        prefs[day] = .training
        kinds[day] = first.kind
        if let gym = stack.first(where: { $0.isGym && $0.workoutPlanID != nil }) {
          assigns[day] = gym.workoutPlanID
        } else {
          assigns[day] = nil
        }
      } else {
    ...
    settings.isManualPlan = true
    settings.dayPreferences = prefs
    settings.manualSessionKinds = kinds
    settings.dayAssignments = assigns
    plannerSettings = settings
  }

  // MARK: Wochen-Check (ruhige Intelligenz)

  var weeklyLoadCheck: WeeklyLoadCheck {
    var categorySessions: [ExerciseCategory: Int] = [:]
    var gymCount = 0
    var cardioCount = 0
    var gymDayFlags: [Bool] = []

    for day in Weekday.allCases.sorted(by: { $0.mondayOffset < $1.mondayOffset }) {
      let stack = daySessions[day] ?? []
      var dayHasGym = false
      for session in stack {
        if session.isCardio { cardioCount += 1; continue }
        guard session.isGym else { continue }
        gymCount += 1
        dayHasGym = true
        guard let plan = workoutPlan(for: session) else { continue }
        var hitThisSession: Set<ExerciseCategory> = []
        for exercise in plan.exercises {
          if let category = ExerciseCategory.fromTargetMuscle(exercise.targetMuscle) {
            hitThisSession.insert(category)
          }
        }
        for category in hitThisSession {
          categorySessions[category, default: 0] += 1
        }
      }
      gymDayFlags.append(dayHasGym)
    }

    let goalIsHypertrophy = plannerSettings.goal == .muscleGain
    let coverage: [WeeklyLoadCheck.MuscleCoverage] = categorySessions
      .sorted { $0.key.sortOrder < $1.key.sortOrder }
      .map { category, count in
        let isUnder = goalIsHypertrophy && category.isMajorForFrequency && count < 2
        return WeeklyLoadCheck.MuscleCoverage(muscle: category.rawValue, sessions: count, isUnder: isUnder)
      }

    let nudge = Self.weeklyNudge(
      coverage: coverage,
      gymDayFlags: gymDayFlags,
      gymCount: gymCount,
      goalIsHypertrophy: goalIsHypertrophy
    )

    return WeeklyLoadCheck(coverage: coverage, gymCount: gymCount, cardioCount: cardioCount, nudge: nudge)
  }

  /// Genau ein ruhiger Hinweis (Priorität: Erholung vor Frequenz). `nil`, wenn
  /// alles passt.
  private static func weeklyNudge(
    coverage: [WeeklyLoadCheck.MuscleCoverage],
    gymDayFlags: [Bool],
    gymCount: Int,
    goalIsHypertrophy: Bool
  ) -> String? {
    var streak = 0
    for hasGym in gymDayFlags {
      streak = hasGym ? streak + 1 : 0
      if streak >= 3 {
        return "Drei harte Tage am Stück — plane einen Ruhetag dazwischen."
      }
    }

    if goalIsHypertrophy, gymCount >= 2 {
      if let under = coverage.first(where: { $0.isUnder }) {
        return "\(under.muscle) nur \(under.sessions)× — für Muskelaufbau wären 2× ideal."
      }
    }
