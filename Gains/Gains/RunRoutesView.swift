...
struct RunRoutesTab: View {
  ...
  private var heatmapMap: some View {
    ...
    if tiles.isEmpty {
      // 2026-05-16 (Fertiger-Audit P0-2): Empty-State war eine **München**-
      // Karte mit dunklem Overlay — wenn ein User in Berlin/Hamburg die App
      // öffnet und noch keine Tiles hat, sieht er eine fremde Stadt. Jetzt:
      // tokenkonforme Card mit Lime-Akzent, ohne Geo-Bias.
      ZStack {
        LinearGradient(
        ...
        VStack(spacing: GainsSpacing.s) {
          ...
          Text("Starte einen GPS-Lauf — jede Strecke landet hier als Heatmap-Punkt.")
  ...
  private func heatmapRegion(_ tiles: [RouteHeatmapTile]) -> MKCoordinateRegion {
    ...
    guard let minLat = lats.min(), let maxLat = lats.max(),
      ...
      // 2026-05-16 (Fertiger-Audit P0-2): Geografisch neutraler Fallback
      // (Greenwich/0,0 mit Welt-Span) statt München. Wird nur bei NaN-Edge-
      // cases erreicht — sichtbar wäre dann „die ganze Welt" statt einer
      // fremden Stadt.
  ...
  private func routeRegion(_ route: SavedRoute) -> MKCoordinateRegion {
    // 2026-05-16 (Fertiger-Audit P0-2): Geografisch neutraler Fallback
    // (Greenwich/0,0) statt München — wird nur bei korrupten Persistenz-
