...
struct GymPlanTab: View {
  ...
  /// Wenn true, wird die `weeklyPreferencesSection` (7-Tages-Karten) nicht
  /// gerendert. Sinnvoll, wenn die einbettende View (z. B. `WeekPlanFullscreenView`)
  /// bereits eine eigene Wochenleiste zeigt — verhindert Duplikat-Inhalt.
  var suppressWeeklyPreferences: Bool = false
  ...
  /// 2026-05-14 (Sheet-Race-Hardening): Geparkte Folge-Aktion, gesetzt vom
  /// `WeekdayDetailSheet`, ausgeführt in dessen `onDismiss`.
  @State private var weekdayPostDismiss: WeekdayPostDismissAction?

  var body: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.xl) {
      planStatusCard
      if !suppressWeeklyPreferences {
    ...
    .sheet(
      ...
      onDismiss: {
        // 2026-05-14 (Sheet-Race-Hardening): Race-frei statt `asyncAfter`.
        // SwiftUI hat das Sheet zu diesem Zeitpunkt vollständig abgebaut.
  ...
  ///
  /// Polish-Loop 162 (2026-05-14): Glas-Komposition statt flachem
  /// `Color.white.opacity(0.06)` — bleibt sekundär, gewinnt aber die
  /// gleiche Tiefe wie die Glas-Pillen im Rest der App.
  private func secondaryCtaButton(
    ...
    Button(action: action) {
      ...
      .compositingGroup()
      .shadow(color: GainsColor.shadowRest, radius: 4, y: 1)
    }
    .buttonStyle(.plain)
  }
  ...
  private var weeklyPreferencesSection: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.s) {
      ...
      VStack(spacing: GainsSpacing.s) {
        // weekStart einmal berechnen, an jede Karte weitergeben (7 × weniger
        // redundante Calendar-Operationen).
        let weekStart = currentWeekStart ?? Date()
        HStack(alignment: .top, spacing: GainsSpacing.xs) {
          ForEach(Weekday.allCases) { day in
            weekdayCard(day, weekStart: weekStart)
          }
        }
        plannerLegend
      }
  ...
  private func weekdayCard(_ day: Weekday, weekStart: Date) -> some View {
    ...
    return Button {
      ...
      VStack(spacing: GainsSpacing.xs) {
        ...
        Text(currentWeekDayNumber(for: day, weekStart: weekStart))
          .font(.system(size: 15, weight: .semibold, design: .rounded))
          .foregroundStyle(GainsColor.ink)

        // Polish-Loop 148 (2026-05-14): Status-Circle mit plusLighter-
        // Inner-Light auf completed + isToday-Variante. Hairline-Border
        // gibt jedem Status die gleiche Cockpit-Tiefe.
        ZStack {
          Circle()
          ...
          Circle()
            .fill(
              LinearGradient(
                colors: [
                  Color.white.opacity(isCompleted ? 0.32 : 0.10),
        ...
        .overlay(
          Circle()
            .strokeBorder(
              isCompleted ? GainsColor.lime.opacity(0.5) : GainsColor.border.opacity(0.35),
              lineWidth: GainsBorder.hairline
            )
        )
        .compositingGroup()
        .shadow(color: isCompleted ? GainsColor.lime.opacity(0.22) : .clear, radius: 5, y: 1)
      ...
      .frame(maxWidth: .infinity)
      .background(
        // 2026-05-14 (Polish-Loop 91): Weekday-Card mit Akzent-Glow für
        // den heutigen Tag + Glas-Composition für die anderen.
        ZStack {
          RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
      ...
      .overlay(
        RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
          .strokeBorder(
            isToday
              ? LinearGradient(
                  colors: [GainsColor.lime.opacity(0.85), GainsColor.lime.opacity(0.18)],
                  startPoint: .top,
                  endPoint: .bottom
                )
              : LinearGradient(
                  colors: [GainsColor.border.opacity(0.55), GainsColor.border.opacity(0.35)],
                  startPoint: .top,
                  endPoint: .bottom
                ),
      ...
      .compositingGroup()
      .shadow(
        color: isToday ? GainsColor.lime.opacity(0.20) : .clear,
        radius: isToday ? 8 : 0
      )
  ...
  // Polish-Loop 163 (2026-05-14): Legend-Chips mit Glow-Dot + Hairline-
  // Pille, statt nackter Punkte mit Text. Konsistent zum Pulse-Vokabular
  // im Rest der App.
  private func legendChip(color: Color, label: String) -> some View {
    ...
    .padding(.vertical, 4)
    .background(
      Capsule().fill(GainsColor.background.opacity(0.55))
    )
    .overlay(
      Capsule().strokeBorder(GainsColor.border.opacity(0.40), lineWidth: GainsBorder.hairline)
    )
    .clipShape(Capsule())
  }
  ...
  // passt.
  /// Einmal pro body-Auswertung berechnet — wird von `currentWeekRangeLabel`,
  /// `currentWeekDayNumber` (7×) und `dateForCurrentWeek` geteilt.
  ...
  private var currentWeekRangeLabel: String {
    ...
    return "KW \(week) · \(Self.weekRangeFormatter.string(from: weekStart)) – \(Self.weekRangeFormatter.string(from: weekEnd))"
  }

  private func currentWeekDayNumber(for day: Weekday, weekStart: Date) -> String {
    let cal = Self.plannerCalendar
    guard let date = cal.date(byAdding: .day, value: day.mondayOffset, to: weekStart) else {
      return "—"
    }
  ...
  private var assignmentsSection: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.s) {
      ...
      if store.scheduledPlannerDays.isEmpty {
        // 2026-05-14 (Empty-State-Sweep): CTA öffnet den Wizard, der genau
        // den nächsten Schritt liefert.
        EmptyStateView(
          style: .inline,
          title: "Noch keine Trainingstage",
          message: "Lass den Wizard deinen Wochenrhythmus vorschlagen — oder setze oben Tage von Hand.",
          icon: "calendar",
          actionLabel: "Wizard starten",
          action: { showsPlanWizard = true }
        )
      } else {
  ...
  private func assignmentRow(_ day: Weekday) -> some View {
    ...
    return HStack(spacing: GainsSpacing.m) {
      // Polish-Loop 169 (2026-05-14): Day-Badge in assignmentRow mit
      // plusLighter-Inner-Light + Hairline-Border + Today-Glow.
      ZStack {
        Circle()
        ...
        Circle()
          .fill(
            LinearGradient(
              colors: [
                Color.white.opacity(isToday ? 0.28 : 0.08),
                Color.white.opacity(0.00)
              ],
              startPoint: .top,
              endPoint: .center
      ...
      .overlay(
        Circle()
          .strokeBorder(
            isToday ? GainsColor.lime.opacity(0.55) : GainsColor.border.opacity(0.40),
            lineWidth: GainsBorder.hairline
          )
      )
      .compositingGroup()
      .shadow(color: isToday ? GainsColor.lime.opacity(0.28) : .clear, radius: 6, y: 1)
  ...
  private func previewDayCell(_ day: GymPlanPreviewDay) -> some View {
    ...
    return Button {
      ...
      VStack(spacing: GainsSpacing.xxs) {
        // 2026-05-14 (Polish-Loop 115): Preview-Day-Cell mit Inner-
        // Light bei Today/Completed + dezenter Halo bei Today.
        ZStack {
          Circle().fill(bg)
        ...
        .compositingGroup()
        .shadow(
          color: day.isToday ? GainsColor.lime.opacity(0.32) : .clear,
