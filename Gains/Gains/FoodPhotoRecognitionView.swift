...
struct FoodPhotoRecognitionSheet: View {
  ...
  // Fix 2026-05-16: Task-Referenz speichern damit bei schnellem
  // Doppel-Pick der erste loadTransferable-Task cancelt wird,
  // bevor der zweite startAnalysis() aufruft (Race-Condition).
  ...
  var body: some View {
    NavigationStack {
      ...
      // 2026-05-15 (Audit-Loop 30): `pickerItem` wurde nach dem Laden
      // nicht zurückgesetzt. Effekt: Wer dasselbe Foto nach einem Retry
      // erneut aus der Galerie wählte, sah keine Reaktion — `onChange`
      // feuert nicht, wenn der Wert identisch zum vorherigen ist. Jetzt
      // wird `pickerItem` nach Verarbeitung explizit auf nil zurückgesetzt.
      .onChange(of: pickerItem) { _, newItem in
        guard let newItem else { return }
        // Fix 2026-05-16: Vorherigen load-Task canceln — bei schnellem
        // Doppel-Pick können sonst zwei Tasks parallel startAnalysis()
        // aufrufen und Ergebnisse überschreiben sich gegenseitig.
        ...
        PhotosPicker(selection: $pickerItem, matching: .images) {
          HStack(spacing: GainsSpacing.s) {
            Image(systemName: "photo.on.rectangle.angled")
              ...
              .frame(width: 44, height: 44)
              .background(GainsColor.accentCool.opacity(0.12))
              .clipShape(RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous))
  ...
  private func analyzingView(image: UIImage) -> some View {
    VStack(spacing: GainsSpacing.xl) {
      ...
      Image(uiImage: image)
        ...
        .shadow(color: GainsColor.lime.opacity(0.15), radius: 16)
  ...
  // damit der User weiß welche Engine gerade aktiv ist. Reihenfolge:
  // Gemini (Cloud, nur wenn Key konfiguriert) → Apple-Vision-Fallback.
  //
  // 2026-05-16 (Release-Audit Cleanup): Apple-Foundation-Models-Pfad wurde
  // entfernt — die `Image`-Input-API ist im aktuellen iOS-26-SDK noch nicht
  // stabil und der gesamte Pfad lieferte nur `unavailable`-Errors. Wenn
  // Apple die finale API stabilisiert, hier wieder einsetzen.
...
private struct PhotoSuggestionCard: View {
      ...
      HStack(spacing: GainsSpacing.s) {
        ...
        VStack(alignment: .leading, spacing: GainsSpacing.xxs) {
          ...
          HStack(spacing: GainsSpacing.xsPlus) {
            ...
            macroTag("K: \(Int(displayCarbsPer100g))g", color: GainsColor.accentCool)
            macroTag("F: \(Int(displayFatPer100g))g", color: GainsColor.macroFat)
            Text("pro 100g")
  ...
  private func confidenceChip(_ confidence: Double) -> some View {
    ...
    let color: Color = confidence > 0.7 ? GainsColor.lime : confidence > 0.4 ? GainsColor.macroFat : GainsColor.border
    return Text("\(pct)%")
...
private struct ScanningDotsView: View {
  ...
  var body: some View {
    ...
    // 2026-05-15 (Audit-Loop 8): Timer-basierte Animation auf `.task`
    // umgestellt — SwiftUI cancelt die Task automatisch beim Dismiss.
    // Vorher: `Timer.scheduledTimer` + manuelles `invalidate()` in
    // `onDisappear`. Funktional ok, aber bei zwei Sheet-Toggles in Folge
    // konnte ein zweiter Timer angelegt werden, bevor der erste invalidate
    // gelaufen war. Mit `.task` ist die View-Lifetime die einzige Quelle
    // der Wahrheit — pro View-Identity läuft genau eine Loop.
    .task {
...
enum FoodImageAnalyzer {
  ...
  static func analyze(image: UIImage,
    ...
    // 2026-05-16 (Release-Audit Cleanup): Apple-Foundation-Models-PRIO-Stufe
    // entfernt — die `Image`-Input-API ist im aktuellen iOS-26-SDK noch nicht
    // stabil. Wenn Apple die Final-API freigibt, hier als PRIO 1 wieder
    // einsetzen (vor Gemini), siehe Git-History.
