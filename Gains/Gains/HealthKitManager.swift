...
final class HealthKitManager: ObservableObject {
  ...
  func requestAuthorization(completion: @escaping (Bool, String?) -> Void) {
    ...
    healthStore.requestAuthorization(toShare: writeTypes, read: readTypes) { success, error in
      DispatchQueue.main.async {
        // 2026-05-14 (Audit-Loop 6): Bei fehlgeschlagener Autorisierung
        // einen ErrorPresenter-Banner zeigen — vorher hat der User
        // schlicht keine Rückmeldung bekommen, wenn HK gesperrt war.
        if !success {
          Task { @MainActor in
            GainsErrorPresenter.shared.present(.healthKitDenied)
          }
        }
  ...
  func saveRunWorkout(
    ...
    let builder = HKWorkoutBuilder(
      healthStore: healthStore,
      configuration: workoutConfiguration(activity: activity, isIndoor: isIndoor),
      device: .local()
    )
    var metadata: [String: Any] = [HKMetadataKeyWorkoutBrandName: "Gains"]
    if !title.isEmpty {
  ...
  private func workoutConfiguration(
