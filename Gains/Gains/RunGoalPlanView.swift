...
struct RunGoalPlannerSection: View {
  ...
  /// 2026-05-14 (Sheet-Race-Hardening): Detail→Setup-Wechsel über onDismiss,
  /// kein `asyncAfter` mehr.
  @State private var pendingShowSetupAfterDetail = false
  ...
  var body: some View {
    ...
    .sheet(
      ...
      onDismiss: {
        // 2026-05-14 (Sheet-Race-Hardening): Detail-Sheet hat ein onEdit
        // signalisiert → öffnet jetzt deterministisch das Setup-Sheet.
        if pendingShowSetupAfterDetail {
          pendingShowSetupAfterDetail = false
          isShowingSetup = true
        }
      }
    ) {
      if let plan = store.runGoalPlan {
        RunGoalPlanDetailSheet(plan: plan, onEdit: {
          pendingShowSetupAfterDetail = true
