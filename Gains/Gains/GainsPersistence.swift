...
extension WorkoutPlan: Codable {
  enum CodingKeys: String, CodingKey {
    case id, source, title, focus, split, estimatedDurationMinutes, exercises, program
  }
  ...
  init(from decoder: Decoder) throws {
    ...
    self.init(
      ...
      // Backward-compat: alte Persistenz-Dumps ohne `program` → nil.
...
enum PersistenceKey {
  ...
  /// Wochenplaner. Als [String: [PlannedSession]] (Weekday.rawValue als Key).
  static let daySessions          = "gains_daySessions"
...
//
// JSONEncoder/Decoder sind thread-safe für encode/decode (ab Swift 5.7+).
// Statt pro Aufruf eine neue Instanz zu allozieren (bisher: ~35 neue
// Instanzen pro saveAll()-Zyklus + zig decodedLoad-Calls beim Start),
// teilen wir statische Instanzen. Das spart Heap-Allokation, ARC-Overhead
// und Initialisierungs-Kosten auf dem Background-Thread.
private enum GainsCoders {
...
extension UserDefaults {
  func encodedSave<T: Encodable>(_ value: T, forKey key: String) {
    guard let data = try? GainsCoders.encoder.encode(value) else {
      #if DEBUG
  ...
  func decodedLoad<T: Decodable>(_ type: T.Type, forKey key: String) -> T? {
    ...
    return try? GainsCoders.decoder.decode(type, from: data)
  }
  ...
  func decodedLoadLenientUUIDDictionary<T: Decodable>(_ elementType: T.Type, forKey key: String) -> [UUID: [T]]? {
    guard let data = data(forKey: key) else { return nil }
    let decoder = GainsCoders.decoder
