...
final class NotificationsManager {
  ...
  func refreshSchedule(for store: GainsStore) {
    ...
    center.getNotificationSettings { [weak self] settings in
      ...
      // Fix 2026-05-16: Auch bereits ausgelieferte Notifications entfernen,
      // damit das Notification-Center leer bleibt wenn der User den Toggle
      // deaktiviert. Vorher wurden nur pending Requests entfernt.
      guard enabled, authorized else {
        self.center.removePendingNotificationRequests(withIdentifiers: Identifier.all)
        self.center.removeDeliveredNotifications(withIdentifiers: Identifier.all)
        return
      }
