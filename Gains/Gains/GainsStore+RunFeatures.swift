...
extension GainsStore {
  ...
  var routeHeatmapTiles: [RouteHeatmapTile] {
    // Cache: dieser Walk geht über ALLE GPS-Punkte aller Läufe + Routen und
    // wuchs mit jedem Lauf. RunRoutesView las ihn 2× pro Render. Cache wird
    // via runHistory.didSet / savedRoutes.didSet invalidiert (GainsStore.swift).
    if let cached = _cachedHeatmapTiles { return cached }
