...
struct GymPlanWizardSheet: View {
  ...
  private var progressBar: some View {
    // 2026-05-14 (Polish-Loop 95): Wizard-Progress mit Lime-Gradient
    // + Glow auf dem Fortschritts-Anteil.
    GeometryReader { geo in
      let ratio = Double(step + 1) / Double(totalSteps + 1)
  ...
  private func wizardChoiceRow(
    ...
    Button(action: onTap) {
          ...
          .background(
            ZStack {
              RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
                .fill(isSelected ? GainsColor.lime.opacity(0.18) : GainsColor.elevated)
              if isSelected {
                RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
                  .fill(
                    LinearGradient(
      ...
      .padding(GainsSpacing.m)
      .background(
        // 2026-05-14 (Polish-Loop 96): Wizard-ChoiceRow Selected-State
        // mit Akzent-Glow im Background statt nur 6% Tint.
        ZStack {
          isSelected ? GainsColor.lime.opacity(0.05) : GainsColor.card
          if isSelected {
            RadialGradient(
      ...
      .overlay(
        RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
          .strokeBorder(
            isSelected
              ? LinearGradient(
                  colors: [GainsColor.lime.opacity(0.65), GainsColor.lime.opacity(0.18)],
                  startPoint: .top,
                  endPoint: .bottom
                )
              : LinearGradient(
      ...
      .compositingGroup()
      .shadow(color: isSelected ? GainsColor.lime.opacity(0.14) : .clear, radius: 10)
    }
    .buttonStyle(.plain)
  }
