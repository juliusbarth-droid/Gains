...
struct WorkoutHubView: View {
    ...
    if let activeRun = store.activeRun {
      VStack(alignment: .leading, spacing: GainsSpacing.m) {
        ...
        HeroPrimaryCTAButton(
          title: heroCtaTitle(for: activeRun.modality, isLive: true),
          icon: "map.fill",
          action: {
  ...
  private func todayPlannedRow(_ plan: WorkoutDayPlan) -> some View {
    ...
    let actionTitle: String = {
      ...
      if store.activeRun != nil { return kind.isCycling ? "Aktive Tour öffnen" : "Aktiven Lauf öffnen" }
    ...
    return VStack(alignment: .leading, spacing: GainsSpacing.m) {
      ...
      if preview.isEmpty {
        let emptyStateActionLabel: String = {
          if store.activeWorkout != nil { return "Aktives Training öffnen" }
          if store.activeRun != nil { return preferredModality.isCycling ? "Aktive Tour öffnen" : "Aktiven Lauf öffnen" }
        ...
        let emptyStateMessage: String = {
          if store.activeWorkout != nil {
            return "Öffne dein aktives Training, damit du direkt zu Übungen, Sätzen und Pausen zurückkehrst."
          }
          if store.activeRun != nil {
  ...
  private func runActivityCard(_ run: CompletedRunSummary, isLatest: Bool) -> some View {
    let footerActionTitle: String = {
      if store.activeWorkout != nil { return "Aktives Training öffnen" }
      if store.activeRun != nil { return run.modality.isCycling ? "Aktive Tour öffnen" : "Aktiven Lauf öffnen" }
