...
struct ExerciseStrengthProgress: Identifiable {
  ...
  let subtitle: String
  /// Rohwert für Sortierung — vermeidet String-Parsing in jedem sort-Aufruf.
  var _topWeightValue: Double = 0
}
...
struct NutritionEntry: Identifiable, Equatable {
  let id: UUID
  let title: String
  let mealType: RecipeMealType
  let loggedAt: Date
...
struct WorkoutPlan: Identifiable {
  ...
  /// Optionaler Programm-Bucket, in dem die Vorlage angezeigt wird
  /// (z. B. "Upper/Lower 4-Tage", "Push/Pull/Beine", "Full Body 3-Tage",
  /// "Bro-Split 5-Tage"). Custom-Workouts und Legacy-Plans bleiben `nil`
  /// und landen im UI in einem „Klassisch"-Bucket. Backward-Codable.
  let program: String?
...
struct WorkoutSession: Identifiable {
  ...
  // MARK: - Workout-Aggregat (Single-Pass, 2026-05-14)
  //
  // Vorher wurden totalSets, completedSets und totalVolume als drei separate
  // computed properties implementiert (je O(exercises×sets)). Im `statsRow`
  // des WorkoutTrackers wurden alle drei gleichzeitig aufgerufen → 3×O(n²).
  // Jetzt: ein einziger Durchlauf liefert alle drei Werte.
  ...
  var stats: WorkoutStats {
    ...
    return WorkoutStats(completedSets: completed, totalSets: total, totalVolume: volume)
  }

  // Backward-compat Forwarder — externe Call-Sites bleiben identisch.
  var totalSets: Int     { stats.totalSets }
  var completedSets: Int { stats.completedSets }
  var totalVolume: Double { stats.totalVolume }
}
...
struct CompletedRunSummary: Identifiable {
  let id: UUID
  /// Editierbar — User kann einen Lauf im Detail-Sheet umbenennen
  /// (`GainsStore.renameRun`).
  var title: String
  let routeName: String
  let finishedAt: Date
  let distanceKm: Double
...
extension WorkoutPlan {
  ...
  // MARK: - Programm-Buckets
  //
  // Die `program`-Strings unten dienen als stabile Gruppierungs-Schlüssel
  // (Workouts-Tab gruppiert Vorlagen pro Programm). Wenn neue Vorlagen
  // ergänzt werden, einfach den gleichen `program`-Wert wiederverwenden.
  enum Program {
    static let upperLower = "Upper/Lower 4-Tage"
    static let pushPullLegs = "Push/Pull/Beine"
    static let fullBody = "Full Body 3-Tage"
    static let broSplit = "Bro-Split 5-Tage"
  }

  /// Evidenzbasierte Starter-Vorlagen — abgeleitet aus Hypertrophie-
  /// Empfehlungen (Schoenfeld 2016/2017, Helms, Nuckols): 10–20 Sätze pro
  /// Muskelgruppe/Woche, Frequenz 2× pro Woche wo möglich, Compounds
  /// schwerer (5–8 Wdh), Assistenz im Hypertrophie-Bereich (8–12),
  /// Isolation hoch (12–15). Pro Workout 6–8 Übungen, ~60–80 min.
  ///
  /// Reihenfolge der Programme orientiert sich am Einsatzweck:
  /// Upper/Lower (4×) → meiste Studien-Evidenz für Volumen-Verteilung,
  /// PPL (3–6×) → maximale Frequenz für Fortgeschrittene,
  /// Full Body (3×) → effizient für Einsteiger/Zeitknappe,
  /// Bro-Split (5×) → klassisch, populär.
  static let starterTemplates: [WorkoutPlan] = upperLowerProgram
    + pushPullLegsProgram
    + fullBodyProgram
    + broSplitProgram

  // MARK: Upper/Lower 4-Tage
  //
  // Klassischer 4-Tage-Split (z. B. Mo/Di/Do/Fr). Jede Muskelgruppe wird
  // 2× pro Woche getroffen — Studienlage favorisiert das ggü. 1×/Woche
  // (Schoenfeld et al. 2016 meta). A-Tage liegen schwerer (Stärke-Bias),
  // B-Tage volumiger mit mehr Isolation.

  private static let upperLowerProgram: [WorkoutPlan] = [
    WorkoutPlan(
      title: "Upper A • Stärke",
      focus: "Brust / Rücken — schwer",
      split: "Upper",
      estimatedDurationMinutes: 75,
      exercises: [
        templateExercise(named: "Bankdrücken", sets: 4, reps: 6),
        templateExercise(named: "Langhantelrudern", sets: 4, reps: 6),
        templateExercise(named: "Schulterdrücken Kurzhantel", sets: 3, reps: 8),
        templateExercise(named: "Klimmzüge", sets: 3, reps: 8),
        templateExercise(named: "Langhantel Curls", sets: 3, reps: 10),
        templateExercise(named: "Trizeps Pushdown (Seil)", sets: 3, reps: 12),
        templateExercise(named: "Face Pulls", sets: 3, reps: 15),
      ],
      program: Program.upperLower
    ),
    WorkoutPlan(
      title: "Lower A • Stärke",
      focus: "Quads / Posterior — schwer",
      split: "Lower",
      estimatedDurationMinutes: 70,
      exercises: [
        templateExercise(named: "Kniebeugen", sets: 4, reps: 6),
        templateExercise(named: "Rumänisches Kreuzheben", sets: 4, reps: 8),
        templateExercise(named: "Beinpresse 45°", sets: 3, reps: 10),
        templateExercise(named: "Beinbeuger sitzend", sets: 3, reps: 10),
        templateExercise(named: "Wadenheben stehend", sets: 4, reps: 12),
        templateExercise(named: "Hängendes Beinheben", sets: 3, reps: 12),
      ],
      program: Program.upperLower
    ),
    WorkoutPlan(
      title: "Upper B • Volumen",
      focus: "Brust oben / Schulter / Arme",
      split: "Upper",
      estimatedDurationMinutes: 75,
      exercises: [
        templateExercise(named: "Schrägbank Kurzhantel", sets: 4, reps: 10),
        templateExercise(named: "Latzug breit", sets: 4, reps: 10),
        templateExercise(named: "Seitheben Kurzhantel", sets: 4, reps: 12),
        templateExercise(named: "Rudern sitzend Kabel", sets: 3, reps: 10),
        templateExercise(named: "Hammer Curls", sets: 3, reps: 12),
        templateExercise(named: "Trizeps Dips", sets: 3, reps: 10),
        templateExercise(named: "Reverse Flys Maschine", sets: 3, reps: 15),
      ],
      program: Program.upperLower
    ),
    WorkoutPlan(
      title: "Lower B • Volumen",
      focus: "Glutes / Beinbizeps / Quads",
      split: "Lower",
      estimatedDurationMinutes: 72,
      exercises: [
        templateExercise(named: "Frontkniebeugen", sets: 4, reps: 8),
        templateExercise(named: "Hip Thrust", sets: 4, reps: 10),
        templateExercise(named: "Ausfallschritte", sets: 3, reps: 10),
        templateExercise(named: "Beinstrecker", sets: 3, reps: 12),
        templateExercise(named: "Beinbeuger liegend", sets: 3, reps: 12),
        templateExercise(named: "Wadenheben sitzend", sets: 4, reps: 15),
      ],
      program: Program.upperLower
    ),
  ]

  // MARK: Push/Pull/Beine
  //
  // PPL skaliert von 3×/Woche (jeweils 1× Push/Pull/Legs) bis 6×/Woche
  // (jeweils 2×) — beliebter Split bei Fortgeschrittenen, weil sich
  // Volumen sauber verteilen lässt. A/B-Variante macht Wiederholung
  // bei 6 Tagen interessanter und verschiebt den Akzent (Brust→Schulter,
  // Breite→Dicke, Quads→Posterior Chain).

  private static let pushPullLegsProgram: [WorkoutPlan] = [
    WorkoutPlan(
      ...
      exercises: [
        templateExercise(named: "Bankdrücken", sets: 4, reps: 6),
        templateExercise(named: "Schrägbank Kurzhantel", sets: 3, reps: 10),
        templateExercise(named: "Schulterdrücken Kurzhantel", sets: 3, reps: 8),
        templateExercise(named: "Seitheben Kurzhantel", sets: 4, reps: 12),
        templateExercise(named: "Trizeps Pushdown (Seil)", sets: 3, reps: 10),
        templateExercise(named: "Skull Crusher", sets: 3, reps: 10),
      ],
      program: Program.pushPullLegs
    ),
    WorkoutPlan(
      title: "Pull A • Breite",
      focus: "Latissimus / Bizeps",
      split: "Pull",
      estimatedDurationMinutes: 72,
      exercises: [
        templateExercise(named: "Klimmzüge", sets: 4, reps: 8),
        templateExercise(named: "Langhantelrudern", sets: 4, reps: 8),
        templateExercise(named: "Latzug breit", sets: 3, reps: 10),
        templateExercise(named: "Rudern sitzend Kabel", sets: 3, reps: 10),
        templateExercise(named: "Langhantel Curls", sets: 3, reps: 10),
        templateExercise(named: "Hammer Curls", sets: 3, reps: 12),
        templateExercise(named: "Face Pulls", sets: 3, reps: 15),
    ...
    WorkoutPlan(
      title: "Beine A • Quad-Fokus",
      focus: "Quads / Waden / Core",
      split: "Beine",
      estimatedDurationMinutes: 72,
      exercises: [
        templateExercise(named: "Kniebeugen", sets: 4, reps: 6),
        templateExercise(named: "Beinpresse 45°", sets: 4, reps: 10),
        templateExercise(named: "Ausfallschritte", sets: 3, reps: 10),
        templateExercise(named: "Beinstrecker", sets: 3, reps: 12),
        templateExercise(named: "Wadenheben stehend", sets: 4, reps: 12),
        templateExercise(named: "Hängendes Beinheben", sets: 3, reps: 12),
    ...
    WorkoutPlan(
      ...
      focus: "Schulter / Brust oben / Trizeps",
      split: "Push",
      estimatedDurationMinutes: 70,
      exercises: [
        templateExercise(named: "Schulterdrücken Langhantel", sets: 4, reps: 6),
        templateExercise(named: "Schrägbankdrücken Langhantel", sets: 3, reps: 8),
        templateExercise(named: "Seitheben Kabel", sets: 4, reps: 15),
        templateExercise(named: "Kurzhantel Flys", sets: 3, reps: 12),
        templateExercise(named: "Overhead Trizeps Kurzhantel", sets: 3, reps: 10),
        templateExercise(named: "Trizeps Pushdown (gerade)", sets: 3, reps: 12),
      ],
      program: Program.pushPullLegs
    ),
    WorkoutPlan(
      title: "Pull B • Dicke",
      focus: "Rücken / Trapez / Bizeps",
      split: "Pull",
      estimatedDurationMinutes: 72,
      exercises: [
        templateExercise(named: "Kreuzheben", sets: 3, reps: 5),
        templateExercise(named: "T-Bar Rudern", sets: 4, reps: 8),
        templateExercise(named: "Pendlay Rudern", sets: 3, reps: 8),
        templateExercise(named: "Latzug eng", sets: 3, reps: 10),
        templateExercise(named: "Predigerbank Curls", sets: 3, reps: 10),
        templateExercise(named: "Konzentrationscurls", sets: 3, reps: 12),
        templateExercise(named: "Shrugs Kurzhantel", sets: 3, reps: 12),
      ],
      program: Program.pushPullLegs
    ),
    WorkoutPlan(
      title: "Beine B • Posterior Chain",
      focus: "Glutes / Beinbizeps / Rücken",
      split: "Beine",
      estimatedDurationMinutes: 72,
      exercises: [
        templateExercise(named: "Rumänisches Kreuzheben", sets: 4, reps: 8),
        templateExercise(named: "Hip Thrust", sets: 4, reps: 10),
        templateExercise(named: "Beinbeuger liegend", sets: 4, reps: 10),
        templateExercise(named: "Bulgarian Split Squat", sets: 3, reps: 10),
        templateExercise(named: "Hyperextensions", sets: 3, reps: 12),
        templateExercise(named: "Wadenheben sitzend", sets: 4, reps: 15),
      ],
      program: Program.pushPullLegs
    ),
  ...
  // MARK: Full Body 3-Tage
  //
  // Mo/Mi/Fr-Klassiker, Greg-Nuckols-Style: jeder Tag trifft alle
  // Major-Patterns (Squat, Hinge, Push, Pull). Optimal für Einsteiger
  // (höchste Frequenz für motorisches Lernen) und Zeitknappe — ein
  // Workout deckt mehr Muskeln ab, weniger Sessions pro Woche nötig.

  private static let fullBodyProgram: [WorkoutPlan] = [
    WorkoutPlan(
      title: "Full Body A • Squat-Bank-Row",
      focus: "Ganzkörper — Squat-Bias",
      split: "Full Body",
      estimatedDurationMinutes: 70,
      exercises: [
        templateExercise(named: "Kniebeugen", sets: 4, reps: 6),
        templateExercise(named: "Bankdrücken", sets: 4, reps: 6),
        templateExercise(named: "Langhantelrudern", sets: 3, reps: 8),
        templateExercise(named: "Seitheben Kurzhantel", sets: 3, reps: 12),
        templateExercise(named: "Langhantel Curls", sets: 3, reps: 10),
        templateExercise(named: "Hängendes Beinheben", sets: 3, reps: 12),
      ],
      program: Program.fullBody
    ),
    WorkoutPlan(
      title: "Full Body B • Deadlift-OHP-Pull",
      focus: "Ganzkörper — Hinge-Bias",
      split: "Full Body",
      estimatedDurationMinutes: 72,
      exercises: [
        templateExercise(named: "Kreuzheben", sets: 3, reps: 5),
        templateExercise(named: "Schulterdrücken Langhantel", sets: 3, reps: 6),
        templateExercise(named: "Klimmzüge", sets: 3, reps: 8),
        templateExercise(named: "Schrägbank Kurzhantel", sets: 3, reps: 10),
        templateExercise(named: "Trizeps Pushdown (Seil)", sets: 3, reps: 10),
        templateExercise(named: "Face Pulls", sets: 3, reps: 15),
    ...
    WorkoutPlan(
      title: "Full Body C • Hip-Thrust-Press-Row",
      focus: "Ganzkörper — Glute/Push-Bias",
      split: "Full Body",
      estimatedDurationMinutes: 70,
      exercises: [
        templateExercise(named: "Hip Thrust", sets: 4, reps: 8),
        templateExercise(named: "Schrägbankdrücken Langhantel", sets: 3, reps: 8),
        templateExercise(named: "Rudern sitzend Kabel", sets: 3, reps: 10),
        templateExercise(named: "Ausfallschritte", sets: 3, reps: 10),
        templateExercise(named: "Hammer Curls", sets: 3, reps: 12),
        templateExercise(named: "Reverse Flys Kurzhantel", sets: 3, reps: 15),
      ],
      program: Program.fullBody
    ),
  ]

  // MARK: Bro-Split 5-Tage
  //
  // Klassischer 5-Tage-Split mit einer Muskelgruppe pro Tag. Studien-
  // Evidenz favorisiert höhere Frequenz für natürliche Lifter, aber das
  // Format ist populär, motivierend und funktioniert solange das
  // Wochen-Volumen stimmt (10–20 Sätze pro Muskel).

  private static let broSplitProgram: [WorkoutPlan] = [
    WorkoutPlan(
      title: "Brust-Tag",
      focus: "Brust komplett",
      split: "Brust",
      estimatedDurationMinutes: 70,
      exercises: [
        templateExercise(named: "Bankdrücken", sets: 4, reps: 6),
        ...
        templateExercise(named: "Schrägbank Kurzhantel", sets: 3, reps: 10),
        templateExercise(named: "Kurzhantel Flys", sets: 3, reps: 12),
        templateExercise(named: "Pec Deck (Butterfly)", sets: 3, reps: 12),
        templateExercise(named: "Brust-Dips", sets: 3, reps: 10),
      ],
      program: Program.broSplit
    ),
    WorkoutPlan(
      title: "Rücken-Tag",
      focus: "Latissimus / Trapez / unterer Rücken",
      split: "Rücken",
      estimatedDurationMinutes: 75,
      exercises: [
        templateExercise(named: "Kreuzheben", sets: 3, reps: 5),
        ...
        templateExercise(named: "Latzug breit", sets: 3, reps: 10),
        templateExercise(named: "Rudern sitzend Kabel", sets: 3, reps: 10),
        templateExercise(named: "Pullover Kurzhantel", sets: 3, reps: 12),
        templateExercise(named: "Face Pulls", sets: 3, reps: 15),
      ],
      program: Program.broSplit
    ),
    WorkoutPlan(
      title: "Schulter-Tag",
      focus: "Alle drei Schulterköpfe + Trapez",
      split: "Schulter",
      estimatedDurationMinutes: 65,
      exercises: [
        templateExercise(named: "Schulterdrücken Langhantel", sets: 4, reps: 6),
        templateExercise(named: "Arnold Press", sets: 3, reps: 10),
        templateExercise(named: "Seitheben Kurzhantel", sets: 4, reps: 15),
        templateExercise(named: "Seitheben Kabel", sets: 3, reps: 15),
        templateExercise(named: "Reverse Flys Kurzhantel", sets: 3, reps: 15),
        templateExercise(named: "Shrugs Kurzhantel", sets: 3, reps: 12),
      ],
      program: Program.broSplit
    ...
    WorkoutPlan(
      title: "Bein-Tag",
      focus: "Quads / Posterior / Waden",
      split: "Beine",
      estimatedDurationMinutes: 80,
      exercises: [
        templateExercise(named: "Kniebeugen", sets: 4, reps: 6),
        templateExercise(named: "Beinpresse 45°", sets: 4, reps: 10),
        templateExercise(named: "Rumänisches Kreuzheben", sets: 3, reps: 8),
        templateExercise(named: "Beinstrecker", sets: 3, reps: 12),
        templateExercise(named: "Beinbeuger sitzend", sets: 3, reps: 12),
        templateExercise(named: "Wadenheben stehend", sets: 4, reps: 15),
        templateExercise(named: "Wadenheben sitzend", sets: 3, reps: 15),
      ],
      program: Program.broSplit
    ),
    WorkoutPlan(
      title: "Arm-Tag",
      focus: "Bizeps / Trizeps komplett",
      split: "Arme",
      estimatedDurationMinutes: 60,
      exercises: [
        templateExercise(named: "Langhantel Curls", sets: 4, reps: 8),
        templateExercise(named: "Skull Crusher", sets: 4, reps: 8),
        templateExercise(named: "Hammer Curls", sets: 3, reps: 10),
        templateExercise(named: "Trizeps Pushdown (Seil)", sets: 3, reps: 10),
        templateExercise(named: "Predigerbank Curls", sets: 3, reps: 10),
        templateExercise(named: "Overhead Trizeps Kurzhantel", sets: 3, reps: 12),
      ],
      program: Program.broSplit
    ),
  ]
  ...
  /// Erzeugt eine Übungs-Vorlage aus dem Katalog. Wenn `sets`/`reps`
  /// angegeben sind, überschreiben sie die Library-Defaults — so kann
  /// dieselbe Übung in einem Stärke- und einem Volumen-Plan unter-
  /// schiedliche Sätze/Wiederholungen tragen, ohne im Katalog mehrfach
  /// zu existieren.
  private static func templateExercise(
    ...
    let setCount = sets ?? exercise.defaultSets
    let repCount = reps ?? exercise.defaultReps
    ...
    return WorkoutExerciseTemplate(
      ...
      sets: Array(
        repeating: WorkoutSetTemplate(
          reps: repCount, suggestedWeight: exercise.suggestedWeight),
        count: setCount
      )
    )
  }
...
// MARK: - PlannedSession
//
// 2026-05-30 — Fundament für den vereinheitlichten Wochenplaner.
//
// Ein Tag ist nicht mehr „eine Session", sondern ein geordneter Stack aus
// `PlannedSession`. Damit werden Hybrid-Tage (Gym + Kardio am selben Tag)
// erstmals darstellbar — z. B. Montag = Push Day UND Easy Run.
//
// Quelle der Wahrheit für den neuen Planer ist `GainsStore.daySessions`.
// Die alten Plan-Felder (`plannerSettings.dayAssignments` /
// `manualSessionKinds` / `dayPreferences`) werden über `syncLegacyMirror()`
// weiter als „Spiegel" der jeweils ersten Session pro Tag gepflegt, damit
// Today-Tab, 4-Wochen-Vorschau & Co. unverändert funktionieren.
//
// Stage 1 deckt Gym (`.strength` + optional `workoutPlanID`) und Lauf-Kardio
// (Run-Kinds aus `PlannedSessionKind`) ab.

struct PlannedSession: Identifiable, Equatable, Codable {
  let id: UUID
  /// Art der Session. `.strength` → Gym, Run-Kinds → Kardio.
  var kind: PlannedSessionKind
  /// Nur für Gym-Sessions: konkretes Workout aus `savedWorkoutPlans`.
  /// `nil` = „offen, Workout später zuweisen".
  var workoutPlanID: UUID?

  init(id: UUID = UUID(), kind: PlannedSessionKind, workoutPlanID: UUID? = nil) {
    self.id = id
    self.kind = kind
    self.workoutPlanID = workoutPlanID
  }

  /// Krafttraining (Gym-Lane, Lime-Akzent).
  var isGym: Bool { kind == .strength }
  /// Lauf-Kardio (Kardio-Lane, Moos/Teal-Akzent).
  var isCardio: Bool { kind.isRun }

  static func gym(_ workoutPlanID: UUID? = nil) -> PlannedSession {
    PlannedSession(kind: .strength, workoutPlanID: workoutPlanID)
  }

  static func cardio(_ kind: PlannedSessionKind) -> PlannedSession {
    PlannedSession(kind: kind)
  }
}

// MARK: - Wochen-Check (ruhige Trainings-Intelligenz)
//
// Liest die Wochenlast aus den geplanten Gym-/Kardio-Sessions und liefert eine
// kompakte Muskel-Abdeckung plus maximal einen ruhigen Hinweis. Bewusst
// diskret — kein Nagging, eine Zeile reicht.

struct WeeklyLoadCheck {
  struct MuscleCoverage: Identifiable {
    let id = UUID()
    let muscle: String
    let sessions: Int
    /// True, wenn diese Hauptgruppe fürs aktuelle Ziel unter-abgedeckt ist.
    let isUnder: Bool
  }
  ...
  let gymCount: Int
  let cardioCount: Int
  /// Genau ein ruhiger Hinweis, falls etwas auffällt — sonst `nil`.
  let nudge: String?

  var hasContent: Bool { gymCount > 0 || cardioCount > 0 }
}

// MARK: - targetMuscle → Hauptgruppe

extension ExerciseCategory {
  /// Robustes Keyword-Mapping von einem freien `targetMuscle`-String auf eine
  /// Hauptgruppe (lowercased + contains), damit der Wochen-Check auch bei
  /// eigenen Workouts greift.
  static func fromTargetMuscle(_ raw: String) -> ExerciseCategory? {
    let m = raw.lowercased()
    if m.contains("brust") || m.contains("chest") || m.contains("pec") { return .chest }
    if m.contains("rücken") || m.contains("ruecken") || m.contains("lat")
        || m.contains("back") || m.contains("trapez") || m.contains("rhom") { return .back }
    if m.contains("schulter") || m.contains("delto") || m.contains("shoulder") { return .shoulders }
    if m.contains("bizeps") || m.contains("biceps") { return .biceps }
    if m.contains("trizeps") || m.contains("triceps") { return .triceps }
    if m.contains("glut") || m.contains("gesäß") || m.contains("gesaess") { return .glutes }
    if m.contains("bein") || m.contains("quad") || m.contains("wade")
        || m.contains("hamstring") || m.contains("beinbeuger") || m.contains("leg") { return .legs }
    if m.contains("bauch") || m.contains("core") || m.contains("rumpf") || m.contains("abs") { return .core }
    return nil
  }

  /// Hauptgruppen, deren Frequenz fürs Muskelaufbau-Ziel zählt.
  var isMajorForFrequency: Bool {
    switch self {
    case .chest, .back, .legs, .shoulders: return true
    default: return false
    }
  }
}
