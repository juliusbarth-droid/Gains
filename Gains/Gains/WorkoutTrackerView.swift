...
struct WorkoutTrackerView: View {
    ...
    NavigationStack {
      ZStack(alignment: .bottom) {
        ...
        if let workout = store.activeWorkout {
          ...
          VStack(spacing: GainsSpacing.m) {
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("Trainingsbildschirm schließen")
            .accessibilityValue("Kein aktives Training")
      ...
      .toolbar {
        ToolbarItem(placement: .topBarLeading) {
          ...
          .accessibilityHint("Schließt den Trainingsbildschirm und kehrt zur Trainingsübersicht zurück")
        }
        ...
        ToolbarItem(placement: .topBarTrailing) {
          ...
          .accessibilityValue(store.activeWorkout == nil ? "Kein aktives Training" : "Aktives Training, kann gespeichert, fortgesetzt oder verworfen werden")
          .accessibilityHint(store.activeWorkout == nil ? "Nicht verfügbar, weil aktuell kein Training geöffnet ist" : "Öffnet die Abschlussbestätigung, in der du dein aktives Training speichern, fortsetzen oder verwerfen kannst")
        }
      ...
      .alert("Training abschließen?", isPresented: $isFinishing) {
        ...
        Text("Wenn du das Training speicherst, bleibt dein aktueller Fortschritt erhalten. Wenn du es fortsetzt, kehrst du direkt in dein aktives Training zurück. Wenn du es verwirfst, geht dein aktueller Fortschritt verloren.")
      }
      ...
      .confirmationDialog(
        ...
        let message: String = {
          switch pending {
          case 0:
            return "Wenn du die Übung behältst, kehrst du direkt zu ihr zurück. Wenn du sie überspringst, wird sie nur eingeklappt."
          case 1:
            return "Wenn du die Übung behältst, kehrst du direkt zu deinem offenen Satz zurück. Wenn du sie überspringst, bleibt 1 offener Satz ungezählt und Volumen sowie Stats bleiben korrekt."
          default:
            return "Wenn du die Übung behältst, kehrst du direkt zu deinen offenen Sätzen zurück. Wenn du sie überspringst, bleiben \(pending) offene Sätze ungezählt und Volumen sowie Stats bleiben korrekt."
          }
        }()
        Text(message)
      }
  ...
  private func commandBar(_ workout: WorkoutSession) -> some View {
    ...
    return VStack(alignment: .leading, spacing: GainsSpacing.s) {
      ...
      HStack(alignment: .center, spacing: GainsSpacing.tight) {
        ...
        .accessibilityHint(isEditMode ? "Schließt den Bearbeiten-Modus und kehrt zum Training zurück" : "Öffnet den Bearbeiten-Modus zum Sortieren, Entfernen und Hinzufügen von Übungen")
  ...
  private func editRow(exercise: TrackedExercise, index: Int, total: Int) -> some View {
    ...
    return HStack(spacing: GainsSpacing.s) {
      ...
      .accessibilityHint("Entfernt diese Übung aus deinem aktuellen Training")
    }
    .padding(.horizontal, GainsSpacing.m)
  ...
  private func editMoveButton(
    ...
    .accessibilityHint(enabled ? (symbol == "chevron.up" ? "Verschiebt die Übung eine Position nach oben" : "Verschiebt die Übung eine Position nach unten") : (symbol == "chevron.up" ? "Nicht verfügbar, weil diese Übung bereits ganz oben steht" : "Nicht verfügbar, weil diese Übung bereits ganz unten steht"))
  }
      ...
      if !isCollapsed {
        ...
        HStack(spacing: GainsSpacing.xsPlus) {
          ...
          if !isAllDone {
            ...
            .accessibilityValue("\(exercise.name), \(completed) von \(total) Sätzen erledigt, Übung kann behalten oder übersprungen werden")
            .accessibilityHint("Öffnet die Bestätigung, in der du diese Übung behalten oder überspringen kannst")
          }
        }
      }
    }
  ...
  private func bottomCTA(_ workout: WorkoutSession) -> some View {
    ...
    .accessibilityValue(detail ?? (isComplete ? "Training kann gespeichert, fortgesetzt oder verworfen werden" : isEmpty ? "Noch keine Übung im Training" : isSetActive ? (isActivePendingSet ? "Aktiver Satz, bereit zum Abschließen" : "Aktiver Satz, Timer läuft") : "\(wStats.completedSets) von \(wStats.totalSets) Sätzen, \(wStats.completedExercises) von \(workout.exercises.count) Übungen erledigt"))
    .accessibilityHint(isEmpty ? "Öffnet die Übungsauswahl für dein Training" : isComplete ? "Öffnet die Abschlussbestätigung, in der du dein Training speichern, fortsetzen oder verwerfen kannst" : isSetActive ? (isActivePendingSet ? "Markiert diesen aktiven Satz als abgeschlossen" : "Stoppt den Timer für diesen aktiven Satz") : "Startet den nächsten offenen Satz")
  }
...
private struct CompactSetRow: View {
  ...
  var body: some View {
    ...
    return HStack(spacing: GainsSpacing.xsPlus) {
      ...
      .accessibilityHint(isCompleted ? "Dieser Satz ist bereits als erledigt markiert" : "Markiert diesen offenen Satz als erledigt")
    }
    ...
    HStack(spacing: 0) {
      ...
      VStack(spacing: 1) {
        ...
        TextField("0", text: text)
          ...
          .accessibilityValue("Aktuell \(text.wrappedValue.isEmpty ? "0" : text.wrappedValue) \(unit)\(focusedField == field ? ", Eingabe aktiv" : "")")
          .accessibilityHint(focusedField == field ? "Direkte Eingabe für \(unit) ist aktiv" : "Öffnet die Eingabe für \(unit)")
          .onSubmit(commit)
...
struct GainsUndoSnackbar: View {
  ...
  var body: some View {
    HStack(spacing: GainsSpacing.s) {
      ...
      .buttonStyle(.plain)
      .accessibilityLabel("Letzte Aktion rückgängig machen")
      .accessibilityValue("Rückgängig verfügbar")
      .accessibilityHint("Stellt den zuletzt entfernten oder verschobenen Trainingsschritt wieder her")
    }
    .padding(.horizontal, GainsSpacing.m)
    .padding(.vertical, GainsSpacing.s)
    .background(
