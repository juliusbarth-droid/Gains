    ...
    NavigationStack {
      ...
      .toolbar {
        ...
        ToolbarItem(placement: .topBarTrailing) {
          ...
          .accessibilityValue(store.activeWorkout == nil ? "Kein aktives Workout" : "Aktives Training, bereit zum Beenden")
          .accessibilityHint(store.activeWorkout == nil ? "Nicht verfügbar, weil aktuell kein Workout geöffnet ist" : "Öffnet die Abschlussbestätigung für das aktuelle Workout")
        }
      ...
      .confirmationDialog(
        ...
        let message: String = {
          switch pending {
          case 0:
            return "Alle Sätze sind bereits erledigt. Die Übung wird nur eingeklappt."
          case 1:
            return "1 offener Satz bleibt ungezählt, Volumen und Stats werden nicht verfälscht."
          default:
            return "\(pending) offene Sätze bleiben ungezählt, Volumen und Stats werden nicht verfälscht."
          }
        }()
        Text(message)
      }
    ...
    return VStack(alignment: .leading, spacing: GainsSpacing.s) {
      ...
      if !isCollapsed {
        ...
        HStack(spacing: GainsSpacing.xsPlus) {
          ...
          .accessibilityValue("\(exercise.name), aktuell \(exercise.sets.count) Sätze")
          ...
          .accessibilityValue(exercise.sets.count <= 1 ? "\(exercise.name), nur ein Satz vorhanden" : "\(exercise.name), aktuell \(exercise.sets.count) Sätze")
          .accessibilityHint(exercise.sets.count <= 1 ? "Nicht verfügbar, weil mindestens ein Satz für diese Übung bestehen bleiben muss" : "Entfernt den letzten Satz dieser Übung")
          ...
          .accessibilityValue(exercise.sets.isEmpty ? "\(exercise.name), noch kein Satz vorhanden" : "\(exercise.name), aktuell \(exercise.sets.count) Sätze, bereit zum Wiederholen")
          .accessibilityHint(exercise.sets.isEmpty ? "Nicht verfügbar, weil es für diese Übung noch keinen Satz zum Wiederholen gibt" : "Fügt einen neuen Satz mit den Werten des letzten Satzes hinzu")
          ...
          if !isAllDone {
            ...
            .accessibilityValue("\(exercise.name), \(completed) von \(total) Sätzen erledigt, bereit zum Überspringen")
            .accessibilityHint("Öffnet die Bestätigung zum Überspringen dieser noch offenen Übung")
          }
        }
      }
    }
...
private struct CompactSetRow: View {
  ...
  var body: some View {
    ...
    return HStack(spacing: GainsSpacing.xsPlus) {
      ...
      .accessibilityValue(isCompleted ? "Satz bereits abgeschlossen" : (isTimerRunning ? "Timer läuft" : "Bereit zum Starten"))
      .accessibilityHint(isCompleted ? "Nicht verfügbar, weil dieser Satz bereits abgeschlossen ist" : (isTimerRunning ? "Pausiert den laufenden Satz-Timer" : "Startet den Satz-Timer für diesen Satz"))
      ...
      .accessibilityValue(isCompleted ? "Bereits abgeschlossen" : "Noch offen, bereit zum Abschließen")
      .accessibilityHint(isCompleted ? "Dieser Satz ist bereits als erledigt markiert" : "Markiert diesen Satz als erledigt")
    }
...
struct GainsUndoSnackbar: View {
  ...
  var body: some View {
    HStack(spacing: GainsSpacing.s) {
      ...
      .accessibilityHint("Stellt den zuletzt entfernten oder verschobenen Workout-Schritt wieder her")
    }
    .padding(.horizontal, GainsSpacing.m)
    .padding(.vertical, GainsSpacing.s)
    .background(
