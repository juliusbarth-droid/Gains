...
struct WorkoutTrackerView: View {
    ...
    NavigationStack {
      ZStack(alignment: .bottom) {
        ...
        if let workout = store.activeWorkout {
          ...
          .accessibilityHint("Wähle als Nächstes einen Plan oder eine Übung aus der Bibliothek, um dein aktives Training zu starten")
        }
      }
      ...
      .toolbar {
        ToolbarItem(placement: .topBarLeading) {
          ...
          .accessibilityValue("Aktives Training bleibt erhalten")
          .accessibilityHint("Schließt den aktiven Trainingsbildschirm und kehrt zur Trainingsübersicht zurück")
        }
        ...
        ToolbarItem(placement: .topBarTrailing) {
          ...
          .accessibilityValue(store.activeWorkout == nil ? "Kein aktives Training" : "Dein aktives Training kann gespeichert, fortgesetzt oder verworfen werden")
  ...
  private func commandBar(_ workout: WorkoutSession) -> some View {
    ...
    return VStack(alignment: .leading, spacing: GainsSpacing.s) {
      ...
      HStack(alignment: .center, spacing: GainsSpacing.tight) {
        ...
        .accessibilityValue(isEditMode ? "Bearbeiten-Modus für dein aktives Training aktiv" : "Bearbeiten-Modus für dein aktives Training aus")
        .accessibilityHint(isEditMode ? "Schließt den Bearbeiten-Modus und kehrt zu deinem aktiven Training zurück" : "Öffnet den Bearbeiten-Modus zum Sortieren, Entfernen und Hinzufügen von Übungen in deinem aktiven Training")
  ...
  private func editRow(exercise: TrackedExercise, index: Int, total: Int) -> some View {
    ...
    return HStack(spacing: GainsSpacing.s) {
      ...
      .accessibilityHint("Entfernt diese Übung aus deinem aktiven Training")
    }
    .padding(.horizontal, GainsSpacing.m)
  ...
  private var editDoneBar: some View {
    ...
    .accessibilityValue("Bearbeiten-Modus für dein aktives Training aktiv")
  ...
  private func exerciseCard(
    ...
    return VStack(alignment: .leading, spacing: GainsSpacing.s) {
      ...
      HStack(alignment: .center, spacing: GainsSpacing.s) {
        ...
        .accessibilityHint("Öffnet die Hilfe für diese Übung in deinem aktiven Training")
      ...
      if !isCollapsed {
        ...
        HStack(spacing: GainsSpacing.xsPlus) {
          ...
          .accessibilityHint(exercise.sets.count <= 1 ? "Nicht verfügbar, weil mindestens ein Satz für diese Übung in deinem aktiven Training bestehen bleiben muss" : "Entfernen ist bereit und entfernt den letzten Satz dieser Übung aus deinem aktiven Training")
          ...
          .accessibilityHint(exercise.sets.isEmpty ? "Nicht verfügbar, weil es für diese Übung in deinem aktiven Training noch keinen Satz zum Wiederholen gibt" : "Wiederholung ist bereit und fügt dieser Übung in deinem aktiven Training einen neuen Satz mit den Werten des letzten Satzes hinzu")
  ...
  private func finishedCard(_ workout: WorkoutSession) -> some View {
    ...
    .accessibilityValue("\(s.completedSets) von \(s.totalSets) Sätzen, \(completedExercises) von \(workout.exercises.count) Übungen, \(Int(s.totalVolume)) Kilogramm Volumen, \(sessionTimeString(start: workout.startedAt, now: Date())) Dauer. Dein aktives Training kann gespeichert, fortgesetzt oder verworfen werden")
  ...
  private func bottomCTA(_ workout: WorkoutSession) -> some View {
    ...
    .accessibilityValue(detail ?? (isComplete ? "Dein aktives Training kann gespeichert, fortgesetzt oder verworfen werden" : isEmpty ? "Noch keine Übung im Training" : isSetActive ? (isActivePendingSet ? "Aktiver Satz, bereit zum Abschließen" : "Aktiver Satz, Timer läuft") : "\(wStats.completedSets) von \(wStats.totalSets) Sätzen, \(wStats.completedExercises) von \(workout.exercises.count) Übungen erledigt"))
...
struct GainsUndoSnackbar: View {
  ...
  var body: some View {
    HStack(spacing: GainsSpacing.s) {
      ...
      .accessibilityValue("Rückgängig in deinem aktiven Training verfügbar")
