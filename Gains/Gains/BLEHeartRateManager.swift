...
final class BLEHeartRateManager: NSObject, ObservableObject {
  ...
  /// Stabilität/Akku (2026-05-31): Jeder Scan bekommt eine Generation. Ein
  /// nach `scanTimeout` Sekunden ins Leere laufender Scan stoppt sich selbst,
  /// damit das BLE-Funkmodul nicht unbegrenzt weiterläuft. `stopScanning()`
  /// erhöht die Generation und entwertet so ausstehende Timeouts.
  ...
  func startScanning() {
    ...
    // Auto-Stop, falls der Scan nichts findet und niemand ihn beendet.
    scanGeneration += 1
    let generation = scanGeneration
    DispatchQueue.main.asyncAfter(deadline: .now() + scanTimeout) { [weak self] in
      guard let self,
            self.scanGeneration == generation,
            case .scanning = self.scanState else { return }
      self.stopScanning()
    }
  }

  func stopScanning() {
    scanGeneration += 1   // entwertet ausstehende Auto-Stop-Timeouts
    central.stopScan()
...
extension BLEHeartRateManager: CBCentralManagerDelegate {
  ...
  func centralManager(
    ...
    // Delegate löschen bevor activePeripheral genullt wird – verhindert Callbacks
    // auf einen deallozierten Manager (dangling delegate).
  ...
  func centralManager(
    ...
    if activePeripheral?.identifier == peripheral.identifier {
      let wasConnectedActively = (connectedDevice != nil)
      // Delegate löschen bevor activePeripheral genullt wird – verhindert Callbacks
      // auf einen deallozierten Manager (dangling delegate).
      peripheral.delegate = nil
      activePeripheral = nil
      ...
      liveHeartRate = nil
      scanState = .idle
      // 2026-05-14 (Audit-Loop 6): Wenn der User aktiv verbunden war und
      // der Sensor plötzlich wegbricht (Akku leer, Reichweite), zeigen
      // wir einen Banner. Bei einem manuellen `disconnect()`-Aufruf war
      // `connectedDevice` schon vorher genullt und wir feuern nicht.
      if wasConnectedActively, error != nil {
        Task { @MainActor in
          GainsErrorPresenter.shared.present(.bleDisconnected)
        }
      }
    }
  }
