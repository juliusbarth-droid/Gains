...
struct RunDetailSheet: View {
  ...
  // Umbenennen + Löschen (2026-05-29). `displayTitle` spiegelt den Namen live
  // im offenen Sheet, da `run` ein Werte-Snapshot ist und sich nach dem
  // Store-Update nicht selbst aktualisiert.
    ...
    NavigationStack {
      ...
      .toolbar {
        ...
        ToolbarItem(placement: .topBarTrailing) {
          Menu {
            Button {
              titleDraft = displayTitle.isEmpty ? run.title : displayTitle
      ...
      .onAppear {
        if displayTitle.isEmpty { displayTitle = run.title }
      }
      .alert("Route speichern", isPresented: $showsSaveRouteAlert) {
        ...
        Button("Speichern") {
          ...
          if store.saveRoute(from: run, title: final) != nil {
            ...
            // 2026-05-16 (Fertiger-Audit P0-3): Success-Haptik + Top-Banner
            // statt nur Button-Text-Wechsel. Vorher lief der Save still ab,
            // User war unsicher ob es wirklich gespeichert wurde.
            GainsErrorPresenter.shared.presentSuccess(.routeSaved)
          }
        }
      ...
      .alert("Lauf umbenennen", isPresented: $showsRenameAlert) {
        TextField("Name", text: $titleDraft)
        Button("Sichern") {
          let trimmed = titleDraft.trimmingCharacters(in: .whitespacesAndNewlines)
          guard !trimmed.isEmpty else { return }
          store.renameRun(run.id, to: trimmed)
          displayTitle = trimmed
        }
        Button("Abbrechen", role: .cancel) {}
      } message: {
        Text("Gib deinem Lauf einen eigenen Namen.")
      }
      .confirmationDialog(
        "Diesen Lauf löschen?",
        ...
        Text("Der Lauf wird dauerhaft aus deinem Verlauf entfernt.")
      }
    }
  }
  ...
  private var mapSection: some View {
    ZStack(alignment: .bottomLeading) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        ...
        Text(displayTitle.isEmpty ? run.title : displayTitle)
      ...
      VStack(spacing: 0) {
        ...
        ForEach(Array(run.splits.enumerated()), id: \.element.id) { idx, split in
          ...
          HStack {
            ...
            Text(split.averageHeartRate > 0 ? "\(split.averageHeartRate)" : "–")
              .font(GainsFont.body(13))
              .foregroundStyle(GainsColor.softInk)
