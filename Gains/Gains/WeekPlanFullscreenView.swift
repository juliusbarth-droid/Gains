...
struct WeekPlanFullscreenView: View {
    ...
    NavigationStack {
      ...
      .sheet(
        ...
        onDismiss: {
          ...
          switch action {
          case .startWorkoutTracker: showsWorkoutTracker = true
          case .startRunTracker:     showsRunTracker = true
          }
        }
      ) { selection in
        WeekdayDetailSheet(
  ...
  private func sessionDisplay(_ session: PlannedSession) -> SessionDisplay {
    ...
    let subtitle = template.map { "\(Int($0.targetDistanceKm)) km · \($0.targetPaceLabel)" } ?? "Lauf"
    return SessionDisplay(
      icon: "figure.run",
