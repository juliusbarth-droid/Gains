...
struct RunTrackerView: View {
  ...
  @State private var shouldResumeAfterStopSheet = false
  ...
  var body: some View {
    NavigationStack {
        ...
        ToolbarItem(placement: .topBarLeading) {
          Button(phase == .live ? "Beenden" : "Schließen") {
            ...
            case .setup:
              ...
              shouldResumeAfterStopSheet = false
      ...
      .sheet(
        isPresented: $showsStopSheet,
        onDismiss: {
          if shouldResumeAfterStopSheet, let run = store.activeRun {
            togglePause(run)
          }
          shouldResumeAfterStopSheet = false
          showsStopSheet = false
  ...
  private func presentStopSheet() {
    guard !showsStopSheet else { return }

    let wasPaused = store.activeRun?.isPaused ?? false
    shouldResumeAfterStopSheet = !wasPaused
    if let run = store.activeRun, !wasPaused {
      togglePause(run)
    }
    showsStopSheet = true
  }
  ...
  private func finishRun(title: String, note: String, feel: RunFeel?) {
    syncStoreWithTracker()
    // 2026-05-10 P1-5 follow-up: StopRunSheet erlaubt Save ab 30s, store.finishRun
    // verwirft aber still, wenn distance==0 UND durationMinutes<1. Bei
    // Indoor-Läufen ohne GPS (30–59s) hieß das: Save-Button reagierte nicht.
    // gpsTracker.durationMinutes ist auf Sekunden floored — für die Save-Schwelle
    // hier auf mindestens 1 Minute aufrunden, damit der Store den Lauf annimmt.
    if gpsTracker.elapsedSeconds >= 30,
       (store.activeRun?.durationMinutes ?? 0) < 1 {
      let rounded = max(Int((Double(gpsTracker.elapsedSeconds) / 60).rounded(.up)), 1)
      store.syncActiveRunGPS(
        distanceKm: gpsTracker.trackedDistanceKm,
...
private struct LiveRunView: View {
  ...
  private var indoorDistanceTile: some View {
    VStack(spacing: GainsSpacing.l) {
      ...
      // 2026-05-10 #41: Vorher nur ±0.5 / ±1.0 — beim Indoor-Bike wollen User
      // oft 100m-Korrekturen am Display nachpflegen, dafür war kein Step da.
      // Jetzt drei Granularitäten ±0.1 / ±0.5 / ±1.0, kompaktere Schrift,
      // damit alle 6 Buttons auf einer Zeile bleiben.
      HStack(spacing: GainsSpacing.xs) {
        indoorStepperButton(label: "−1.0", deltaKm: -1.0)
        indoorStepperButton(label: "−0.5", deltaKm: -0.5)
        indoorStepperButton(label: "−0.1", deltaKm: -0.1)
        indoorStepperButton(label: "+0.1", deltaKm: 0.1, accent: true)
