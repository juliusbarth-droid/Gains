...
final class GainsStore: ObservableObject {
  ...
  private func loadPersistedData(today: Date) {
    ...
    // 2026-05-10 P1-3: Pause-Timer-State wiederherstellen, falls die App
    // während einer Set-Pause memory-evicted wurde. Wenn der Endpunkt schon
    // in der Vergangenheit liegt, lädt der View-Task ihn ohnehin still weg —
    // wir müssen hier nicht filtern.
    if ud.object(forKey: PersistenceKey.activeRestTimerEndsAt) != nil {
      let timestamp = ud.double(forKey: PersistenceKey.activeRestTimerEndsAt)
      if timestamp > 0 {
        activeRestTimerEndsAt = Date(timeIntervalSince1970: timestamp)
      }
    }
    if ud.object(forKey: PersistenceKey.activeRestDuration) != nil {
      let stored = ud.integer(forKey: PersistenceKey.activeRestDuration)
      if stored > 0 { activeRestDuration = stored }
    }
  }
  ...
  func saveAll(completion: (() -> Void)? = nil) {
    ...
    let _activeRestTimerEndsAt = self.activeRestTimerEndsAt
    let _activeRestDuration    = self.activeRestDuration
    ...
    DispatchQueue.global(qos: .utility).async {
      ...
      // 2026-05-10 P1-3: Pause-Timer-State persistieren. Nil ⇒ Key entfernen,
      // damit ein abgeschlossener Workout-Pfad keine Geister-Pause beim
      // nächsten Start hinterlässt.
      if let end = _activeRestTimerEndsAt {
        ud.set(end.timeIntervalSince1970, forKey: PersistenceKey.activeRestTimerEndsAt)
      } else {
        ud.removeObject(forKey: PersistenceKey.activeRestTimerEndsAt)
      }
      ud.set(_activeRestDuration, forKey: PersistenceKey.activeRestDuration)
  ...
  func clearAllData() {
    ...
    let allKeys: [String] = [
      ...
      // 2026-05-10 V-P1-9: Goal-Plan und Onboarding-Timestamp gehören mit
      // resettet — sonst landet ein „Reset"-Tester nach Re-Onboarding direkt
      // wieder in einem alten Day-One-Window oder mit altem Goal-Plan.
      PersistenceKey.runGoalPlan,
      GainsKey.onboardingCompletedAt,
      // 2026-05-10 P1-3: Pause-Timer-State auch wegwerfen, sonst startet
      // der Tester nach Reset mit aktiver Pause aus einem fremden Workout.
      PersistenceKey.activeRestTimerEndsAt,
      PersistenceKey.activeRestDuration,
  ...
  var exerciseStrengthProgress: [ExerciseStrengthProgress] {
    exerciseStrengthProgress(from: workoutHistory)
  }

  /// 2026-05-10 G-P1-8: Stärke-Fortschritt zeitraum-aware. Vorher griff der
  /// STATS-Tab immer auf die lifetime-aggregierte Computed-Property zu — der
  /// Zeitraum-Filter (Woche/Monat/All) hatte hier keine Wirkung, während alle
  /// anderen Sektionen filterten. Mit dem expliziten History-Parameter kann
  /// der Caller jetzt seinen gefilterten Slice durchreichen.
  func exerciseStrengthProgress(
    from history: [CompletedWorkoutSummary]
  ) -> [ExerciseStrengthProgress] {
    let grouped = Dictionary(
      grouping: history.flatMap { workout in
  ...
  /// 2026-05-10 Intuitivitäts-Sweep #11: Default-Pause aus dem Planner-Goal
  /// (Hypertrophie/Fettabbau/Performance) statt fixem 150s übernehmen.
  /// Vorher behielt der Tracker seine alte Setting für jedes neue Workout —
  /// User mit Fat-Loss-Plan starteten weiter mit 2:30 Pause statt 1:30.
  /// Nur seeden, wenn keine laufende Pause unterbrochen wird (Defensive).
  private func seedRestDurationFromPlanGoal() {
    guard activeRestTimerEndsAt == nil else { return }
    activeRestDuration = recommendedRestSecondsCompound
  }
