...
struct HomeView: View {
  ...
  @State private var didStartArrangeWorkout = false
  ...
  var body: some View {
    ...
    .sheet(
      ...
      onDismiss: {
        ...
        didStartArrangeWorkout = false
        isShowingWorkoutChooser = false
        runPending(&pendingAfterChooser)
      }
    ) { workoutChooserSheet }
    .sheet(
      ...
      onDismiss: {
        ...
        runPending(&pendingAfterBuilder)
      }
    ) { workoutBuilderSheet }
    .sheet(
      item: $arrangingPlan,
      onDismiss: {
        ...
        didStartArrangeWorkout = false
        arrangingPlan = nil
        runPending(&pendingAfterArrange)
      }
    ) { plan in
  ...
  private func arrangePlanSheet(plan: WorkoutPlan) -> some View {
    WorkoutArrangeView(
      ...
      onStart: {
        ...
        pendingAfterArrange = {
          runCoachAction(.openWorkoutTracker)
          pendingAfterArrange = nil
        }
        didStartArrangeWorkout = false
        arrangingPlan = nil
  ...
  private var pulseStrip: some View {
    // 2026-05-10 #31: Tile-Spacing tight (10) → s (12), damit Tap-Toleranz
    // zwischen drei nebeneinanderliegenden Tap-Targets steigt — vorher
    // konnte ein knapper Daumen am Edge versehentlich auf den Nachbarn
    // springen.
  ...
  private var cockpitSpotlightCard: some View {
    ...
    VStack(alignment: .leading, spacing: 0) {
      ...
      VStack(alignment: .leading, spacing: GainsSpacing.s) {
        ...
        if let week = store.nextFourWeeksSchedule.first(where: { $0.weekIndex == 0 }) {
  ...
  private var weekRing: some View {
    ZStack {
      ...
      // 2026-05-10 P2-6: Fresh-Install zeigte „0 / 0" — der Wochenring sah aus
      // wie ein gescheiterter Datenbezug. Wenn noch kein Wochenziel gesetzt ist
      // (kein Plan / kein Wizard durch), labeln wir den Zustand klar als
      // „Plan offen" statt mit einer Null-Quote.
      VStack(spacing: -2) {
  ...
  private var quickStartTrainingSubtitle: String {
    if let aw = store.activeWorkout {
      let title = aw.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Krafttraining" : aw.title
      if aw.totalSets <= 0 {
        return "Live · \(title)"
      }
      return "\(aw.completedSets)/\(aw.totalSets) Sätze · \(title)"
    }
    if let plan = store.todayPlannedWorkout {
      let title = plan.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Krafttraining" : plan.title
      return "Heute · \(title)"
    }
    if let last = store.lastCompletedWorkout {
      let title = last.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Krafttraining" : last.title
      return "Zuletzt · \(title)"
    }
    return "Plan wählen oder spontan"
  }

  /// Statuszeile für die Lauf-Schnellstart-Kachel — analog zur Training-
  /// Kachel: Live-Lauf > Tagesplan > letzte Session > Default.
  private var quickStartCardioSubtitle: String {
    if let ar = store.activeRun {
      if ar.distanceKm <= 0, ar.durationMinutes <= 0 {
        return "Live · \(ar.modality.displayName)"
      }
      let totalMinutes = max(ar.durationMinutes, 0)
      let durationLabel = totalMinutes >= 60
        ? String(format: "%d:%02d h", totalMinutes / 60, totalMinutes % 60)
        : "\(totalMinutes) min"
      return String(format: "%.1f km · %@", ar.distanceKm, durationLabel)
    }
    if let runTemplate = store.todayPlannedDay.runTemplate {
      let title = runTemplate.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Cardio" : runTemplate.title
      return "Heute · \(title)"
    }
    if let last = store.latestCompletedRun {
      let title = last.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Cardio-Session" : last.title
      return "Zuletzt · \(title)"
    }
  ...
  private func tileContextMenu(for spec: ActionTileSpec) -> some View {
    ...
    case .meal:
      Button {
        runCoachAction(.openNutritionCapture)
      } label: {
  ...
  private var nutritionStatusLabel: String {
    ...
    if store.nutritionTargetCalories <= 0 && store.nutritionTargetProtein <= 0 {
      return "Tracking läuft"
    }
    if store.nutritionTargetCalories <= 0 {
      return "Protein im Fokus"
    }
    if store.nutritionTargetProtein <= 0 {
      return "Kalorien im Fokus"
    }
  ...
  private var nutritionCaptionLine: String {
    ...
    if store.nutritionTargetCalories <= 0 && store.nutritionTargetProtein <= 0 {
      return "Heute sind bereits Mahlzeiten geloggt. Setz Ziele, wenn du genauer steuern willst."
    }
    if store.nutritionTargetCalories <= 0 {
      let remainingProtein = max(store.nutritionTargetProtein - store.nutritionProteinToday, 0)
      if remainingProtein == 0 {
        return "Protein-Ziel erreicht. Setz kcal, wenn du genauer steuern willst."
      }
      return "Noch \(remainingProtein) g Protein offen."
    }
    if store.nutritionTargetProtein <= 0 {
      let remainingKcal = max(store.nutritionTargetCalories - store.nutritionCaloriesToday, 0)
      if remainingKcal == 0 {
        return "Kalorien-Ziel erreicht. Setz Protein, wenn du genauer steuern willst."
      }
      return "Noch \(remainingKcal) kcal offen."
    }
    let remainingKcal = max(store.nutritionTargetCalories - store.nutritionCaloriesToday, 0)
    let remainingProtein = max(store.nutritionTargetProtein - store.nutritionProteinToday, 0)
  ...
  private var currentCoachBrief: CoachBrief {
    ...
    if let ar = store.activeRun {
      ...
      let durationLabel = totalMinutes >= 60
        ? String(format: "%d:%02d h", totalMinutes / 60, totalMinutes % 60)
        : "\(totalMinutes) min"
      let runSubline: String = {
        if ar.isPaused {
          return String(format: "%.2f km bisher · %@ aufgenommen", ar.distanceKm, durationLabel)
        }
        if ar.currentHeartRate > 0 {
          return String(format: "%.2f km · %@ · HF %d bpm", ar.distanceKm, durationLabel, ar.currentHeartRate)
        }
        return String(format: "%.2f km · %@ · Herzfrequenz wird gesucht", ar.distanceKm, durationLabel)
      }()
      return CoachBrief(
        eyebrow: ar.isPaused ? "RUN PAUSIERT" : "RUN LIVE",
        glyph: ar.isPaused ? "pause.circle.fill" : "figure.run",
        accent: ar.isPaused ? GainsColor.mutedInk : GainsColor.ember,
        headline: ar.isPaused
          ? "Lauf pausiert seit \(durationLabel)."
          : "Lauf läuft seit \(durationLabel).",
        subline: runSubline,
        primary: CoachActionDescriptor(
          title: ar.isPaused ? "Run fortsetzen" : "Run öffnen",
          icon: ar.isPaused ? "pause.fill" : "figure.run",
          metric: ar.isPaused ? durationLabel : String(format: "%.2f km", ar.distanceKm),
          action: .openRunTracker
        ),
        secondary: nil
      )
  ...
  private var adaptiveTiles: [ActionTileSpec] {
    ...
    // 2026-05-10 Release-Polish: vorher „Wasser · +250 ml · Tracking" suggerierte
    // einen Hydration-Tracker, der nicht existiert. Tap landete im Nutrition-Tab,
    // wo nichts wassertypisches passiert. Umgewidmet zu einer ehrlichen Tagesbilanz-
    // Shortcut — Mahlzeiten-Tile loggt, dieses Tile öffnet die Tages-Übersicht.
    let waterTile = ActionTileSpec(
      kind: .water,
      eyebrow: "TAGESBILANZ",
      title: "Heute",
      subtitle: "Kalorien & Makros",
  ...
  private func closeAllSheets() {
    ...
    if isShowingWorkoutTracker { isShowingWorkoutTracker = false }
    if isShowingRunTracker { isShowingRunTracker = false }
  ...
  private func startLastWorkoutFromHome() {
    guard store.repeatLastWorkout() else { return }
    runCoachAction(.openWorkoutTracker)
  }
  ...
  private func startRunHubFromHome() {
    closeAllSheets()
    navigation.openTraining(workspace: .laufen)
  }

  private func openWorkoutChooserFromHome() {
    closeAllSheets()
    isShowingRunTracker = false
  ...
  private func presentArrange(for plan: WorkoutPlan) {
    closeAllSheets()
    let startedWorkout = store.activeWorkout == nil
    if startedWorkout {
      pendingActionLock = .startingWorkout
      store.startWorkout(from: plan)
    }
    didStartArrangeWorkout = startedWorkout
    isShowingWorkoutTracker = false
    isShowingRunTracker = false
    arrangingPlan = plan
  }
