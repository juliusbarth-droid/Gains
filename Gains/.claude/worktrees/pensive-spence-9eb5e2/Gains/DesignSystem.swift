...
enum GainsColor {
  ...
  static let limeGlow = Color(red: 0.78, green: 0.92, blue: 0.27, opacity: 0.22)
  ...
  static let coolGlow = Color(red: 0.36, green: 0.83, blue: 0.94, opacity: 0.18)
...
extension View {
  ...
  /// - Hairline-Border (0.6pt) statt klobiger 1pt-Linie
  /// - Subtiler Inner-Highlight oben (verkauft die Tiefe ohne Glow zu erzwingen)
  /// - Kühlerer Drop-Shadow, knapper unter der Card
  ///
  /// A13 (Cleaner-Pass): CornerRadius 20 → 16 (`GainsRadius.standard`),
  /// Drop-Shadow Opacity 0.55 → 0.38 + Radius 18 → 14. Cards lesen sich
  /// damit präziser und weniger wuchtig — näher am modernen iOS-Vokabular.
      ...
      .overlay(
        shape
          .strokeBorder(
            LinearGradient(
              colors: [
                GainsColor.border.opacity(0.85),
  ...
  ///
  /// A13 (Cleaner-Pass): CornerRadius 20 → 16, Glow-Opacity 0.18 → 0.10,
  /// Glow-Radius 16 → 10, Drop-Shadow auf 0.38/14 angepasst (analog
  /// gainsCardStyle). Akzent bleibt sichtbar, aber leiser.
    ...
    return self
      ...
      .overlay(
        shape
          .strokeBorder(
            LinearGradient(
              colors: [accent.opacity(0.7), accent.opacity(0.12)],
              startPoint: .top,
              endPoint: .bottom
            ),
      ...
      .shadow(color: accent.opacity(0.10), radius: 10, x: 0, y: 0)
      .shadow(color: Color.black.opacity(0.38), radius: 14, x: 0, y: 6)
  }
...
struct EmptyStateView: View {
  ...
  private var inlineLayout: some View {
    ...
    .overlay(hairlineBorder(cornerRadius: GainsRadius.standard))
  ...
  private var prominentLayout: some View {
    ...
    .background(GainsColor.card)
    .overlay(hairlineBorder(cornerRadius: GainsRadius.hero))
    .clipShape(RoundedRectangle(cornerRadius: GainsRadius.hero, style: .continuous))
    .shadow(color: Color.black.opacity(0.36), radius: 14, x: 0, y: 6)
  }
  ...
  private func cardLayout(_ resolvedIcon: String?) -> some View {
    ...
    .padding(14)
    .background(GainsColor.card)
    .overlay(hairlineBorder(cornerRadius: GainsRadius.small))
  ...
  private func actionButton(_ label: String, action: @escaping () -> Void) -> some View {
    // A13: Lime-Glow 0.35 → 0.18, Radius 12 → 8 — Sekundär-Action bleibt
    // erkennbar als CTA, glüht aber nicht mehr.
    Button(action: action) {
      ...
      .clipShape(Capsule())
      .shadow(color: GainsColor.lime.opacity(0.18), radius: 8, x: 0, y: 0)
    }
    .buttonStyle(.plain)
  }
}
  ...
  var body: some View {
    ...
    .overlay(
      RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
        .strokeBorder(GainsColor.border.opacity(borderOpacity), lineWidth: GainsBorder.hairline)
    )
    .clipShape(RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous))
  }
...
struct GainsWordmark: View {
  ...
  var body: some View {
    HStack(spacing: 0) {
      ...
      // A13: Glow-Opacity 0.55 → 0.32 — Punkt bleibt der Akzent des Wordmarks,
      // ist aber nicht mehr der hellste Spot im Header.
      Text(".")
        .font(.system(size: size, weight: .semibold))
        .foregroundStyle(GainsColor.lime)
        .tracking(size <= 20 ? -0.4 : -0.8)
        .shadow(color: GainsColor.lime.opacity(0.32), radius: size * 0.22, x: 0, y: 0)
    }
  }
}
...
struct GainsKPIStrip: View {
  ...
  var body: some View {
    ...
    .clipShape(RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous))
    // A13: Akzent-Glow von 0.10/r=18 auf 0.05/r=10 — Strip wirkt sauberer
    // ohne Halo, der mit umliegenden Cards interferiert.
    .shadow(color: accent.opacity(0.05), radius: 10, x: 0, y: 0)
  }
}
...
struct GainsGlowChip: View {
  ...
  var body: some View {
    ...
    .overlay(
      Capsule().strokeBorder(accent.opacity(0.40), lineWidth: GainsBorder.hairline)
    )
    .clipShape(Capsule())
    .shadow(color: accent.opacity(0.18), radius: 8, x: 0, y: 0)
  }
}
...
struct GainsHeroCard<TrailingBadge: View, Footer: View>: View {
  ...
  var body: some View {
    ...
    // A13 (Cleaner-Pass): Hero-Drop-Shadow gedimmt (0.55→0.40, r=22→16, y=10→8).
    // Hero-Cards bleiben „Bühne", aber stehen nicht mehr massiv vom Background ab.
  ...
  private var heroBorder: some View {
    // A13: Lime-Border-Stops gedimmt (0.55→0.40, 0.08→0.05) und Linewidth
    // an `GainsBorder.accent` (0.8) angepasst — dezenter Akzent, keine
    // selbstreferentielle „Lime ringt nach Aufmerksamkeit"-Wirkung mehr.
    RoundedRectangle(cornerRadius: GainsRadius.hero, style: .continuous)
      .strokeBorder(
        LinearGradient(
          colors: [
            GainsColor.lime.opacity(0.40),
            GainsColor.lime.opacity(0.05)
          ],
          startPoint: .top,
          endPoint: .bottom
  ...
  ///
  /// A13: Halo-Spitze 0.18 → 0.10, Höhe 80 → 64. Reicht weiterhin als
  /// „die Card hat oben eine Markierung", ohne aktiv zu glühen.
...
struct GainsHeroStatusBadge: View {
  ...
  var body: some View {
    // A13: Live-Glow 12 → 8 + Opacity 0.6 → 0.32 — der LIVE-Badge bleibt
    // markant, schreit aber nicht mehr durch die Hero-Card.
    Text(label)
      ...
      .clipShape(Capsule())
      .shadow(color: glowColor, radius: tone == .live ? 8 : 0)
  }
...
extension View {
  /// Card-Shadow-Modifier — Drop-in-Ersatz für `.shadow(color:.black.opacity(...))`.
  func gainsCardShadow() -> some View {
    self.shadow(color: Color.black.opacity(0.38), radius: 14, x: 0, y: 6)
  }
  /// Lift-Shadow-Modifier — interaktive Cards.
  func gainsLiftShadow() -> some View {
    self.shadow(color: Color.black.opacity(0.40), radius: 16, x: 0, y: 8)
  }
  /// Hero-Shadow-Modifier — Spotlight-Cards.
  func gainsHeroShadow() -> some View {
    self.shadow(color: Color.black.opacity(0.40), radius: 18, x: 0, y: 10)
  }
  /// Soft-Shadow-Modifier — leichte Surfaces.
  func gainsSoftShadow() -> some View {
    self.shadow(color: Color.black.opacity(0.28), radius: 10, x: 0, y: 4)
  }
    ...
    HStack(spacing: GainsSpacing.s) {
      if let icon {
        ZStack {
          ...
          RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
            .strokeBorder(iconColor.opacity(0.32), lineWidth: GainsBorder.hairline)
          Image(systemName: icon)
...
struct GainsPrimaryButton: View {
  ...
  var body: some View {
    Button(action: action) {
      ...
      .background(
        RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
          .fill(tone.background.opacity(isEnabled ? 1.0 : 0.35))
      )
      .overlay(
        RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
          .strokeBorder(tone.background.opacity(0.45), lineWidth: GainsBorder.accent)
      )
      .gainsAccentGlow(tone.glow, radius: 10)
    }
    .buttonStyle(.plain)
    .disabled(!isEnabled)
  }
...
struct GainsSecondaryButton: View {
  ...
  var body: some View {
    Button(action: action) {
      ...
      .background(
        RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
          .fill(GainsColor.ctaRaised.opacity(0.6))
      )
      .overlay(
        RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.xs) {
      ...
      HStack(alignment: .firstTextBaseline, spacing: 2) {
        Text(value)
          .font(isPrimary ? GainsFont.metricLarge : GainsFont.metric)
          .foregroundStyle(GainsColor.ink)
          .lineLimit(1)
          .minimumScaleFactor(0.7)
        if let unit {
    ...
    .overlay(
      RoundedRectangle(cornerRadius: GainsRadius.standard, style: .continuous)
        .strokeBorder(
          isPrimary ? accent.opacity(0.32) : GainsColor.border.opacity(0.6),
          lineWidth: isPrimary ? GainsBorder.accent : GainsBorder.hairline
        )
    )
    .gainsAccentGlow(isPrimary ? accent : .clear, radius: isPrimary ? 8 : 0)
  }
}
...
extension View {
  ...
  /// Kompakte Card: 12pt-Padding + small Radius + Hairline-Border.
