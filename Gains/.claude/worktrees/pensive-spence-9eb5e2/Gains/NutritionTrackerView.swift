...
struct CalorienTrackerView: View {
  ...
  // 2026-05-10 N-P1-24: Wenn der Barcode-Scanner sich mit „Manuell suchen"
  // verabschiedet, öffnen wir die FoodSearch erst nach Sheet-Dismiss — sonst
  // animiert SwiftUI zwei Sheets gleichzeitig (analog zum FoodSearch-Pattern).
  @State private var pendingAfterBarcode: (() -> Void)? = nil
  ...
  var body: some View {
    ...
    .sheet(
      isPresented: $showsNutritionWizard,
      onDismiss: {
        pendingAfterFoodSearch = nil
        pendingAfterBarcode = nil
        showsFoodSearch = false
    ...
    .sheet(
      isPresented: $showsQuickGoalEdit,
      onDismiss: {
        pendingAfterFoodSearch = nil
        pendingAfterBarcode = nil
        showsFoodSearch = false
    ...
    .sheet(
      isPresented: $showsPhotoRecognition,
      onDismiss: {
        pendingAfterFoodSearch = nil
        pendingAfterBarcode = nil
        pendingMealType = currentMealTypeForTime()
    ...
    .sheet(
      isPresented: $showsBarcodeScanner,
      onDismiss: {
        pendingAfterFoodSearch = nil
        pendingMealType = currentMealTypeForTime()
        showsFoodSearch = false
        ...
        showsPhotoRecognition = false
        showsBarcodeScanner = false
        let action = pendingAfterBarcode
        pendingAfterBarcode = nil
        action?()
      }
    ) {
      BarcodeScannerSheet(
        ...
        onRequestManualSearch: {
          pendingAfterBarcode = { showsFoodSearch = true }
          showsBarcodeScanner = false
        }
      )
      .environmentObject(store)
  ...
  private var dateNavigationRow: some View {
    HStack {
      ...
      // 2026-05-10 N-P1-26: Vorwärts-Chevron öffnet jetzt bis zu 7 Tage in
      // die Zukunft. Use-Case Meal-Prep — User möchte Mahlzeiten für die
      // nächsten Tage planen/voreintragen. Mehr als +7 Tage ist out-of-scope
      // für ein Tagebuch und würde die History-Lookups verfälschen.
      Button {
        withAnimation(.spring(response: 0.3)) {
          let tomorrow = Calendar.current.date(byAdding: .day, value: 1, to: selectedDate) ?? selectedDate
          let todayStart = Calendar.current.startOfDay(for: Date())
          let maxFuture = Calendar.current.date(byAdding: .day, value: 7, to: todayStart) ?? todayStart
          if tomorrow <= maxFuture {
        ...
        Image(systemName: "chevron.right")
          .font(.system(size: 14, weight: .semibold))
          .foregroundStyle(canNavigateForward ? GainsColor.softInk : GainsColor.border)
          .frame(width: 36, height: 36)
          .background(GainsColor.elevated)
          .clipShape(Circle())
      }
  ...
  /// True solange `selectedDate` weniger als 7 Tage in der Zukunft liegt
  /// (siehe N-P1-26).
  private var canNavigateForward: Bool {
    let todayStart = Calendar.current.startOfDay(for: Date())
    guard let maxFuture = Calendar.current.date(byAdding: .day, value: 7, to: todayStart),
          let next = Calendar.current.date(byAdding: .day, value: 1, to: selectedDate)
    else { return false }
    return next <= maxFuture
  }
