...
struct OnboardingView: View {
  ...
  private var profileStep: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.xl) {
      ...
      // Name
      // 2026-05-10 #32: Subtile Validation-Border — sobald der Nutzer einen
      // sinnvollen Namen tippt, wird der Border lime, damit klar ist „Eingabe
      // OK". Default-Border bleibt neutral, kein roter Fehler-State (den triggern
      // wir nicht, weil Name leer im Code via .nilIfEmpty toleriert wird).
      VStack(alignment: .leading, spacing: GainsSpacing.xsPlus) {
        ...
        TextField("Wie sollen wir dich nennen?", text: $name)
          ...
          .overlay(
            RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous)
              .stroke(
                isNameValid ? GainsColor.lime.opacity(0.6) : GainsColor.border.opacity(0.5),
                lineWidth: isNameValid ? 1.4 : 1
              )
          )
          .clipShape(RoundedRectangle(cornerRadius: GainsRadius.small, style: .continuous))
          .submitLabel(.next)
  ...
  /// Mindestens 2 Zeichen, getrimmt — verhindert dass „ " allein als gültig
  /// durchgeht (siehe #32, Validation-Border am Name-Feld).
  ...
  private var nutritionSummaryDetail: String {
    ...
    if kcal > 0 {
      // 2026-05-10 P2-4: Tester sah „2865 kcal" ohne Bezug. Wir zeigen jetzt
      // BMR × Aktivität → Tagesziel zusammen, damit klar ist warum die Zahl
      // bei dem konkreten Profil rauskommt. Mifflin-St-Jeor-Formel — Sex-
      // Konstante (+5 m / -161 w) berücksichtigt.
      let base = 10.0 * Double(weightKg) + 6.25 * Double(heightCm) - 5.0 * Double(age)
      let bmrEstimate = Int((base + (sex == .male ? 5.0 : -161.0)).rounded())
      return
        "Mifflin-St Jeor BMR ~\(bmrEstimate) kcal × \(activityLevel.title) → Tagesziel ~\(kcal) kcal · \(protein) g Protein"
    }
  ...
  private func advance() {
    ...
    if currentStep == .training {
      ...
      // 2026-05-10 Intuitivitäts-Sweep #3: Auch das Nutrition-Profil partial
      // committen, sonst zeigt die Summary noch „Werte werden im Profil
      // berechnet" — kcal/Protein-Targets entstehen erst aus dem Profil. Wir
      // bauen dasselbe Profil wie in finish() und setzen es in den Store, so
      // dass `nutritionTargetCalories`/`nutritionTargetProtein` im
      // Summary-Render echte Zahlen zurückgeben. saveAll passiert weiterhin
      // erst in finish().
      ...
      store.nutritionProfile = profile
    }
