...
struct GymTodayTab: View {
  ...
  private var todayHeroCard: some View {
    ...
    // 2026-05-10 G-P2-3: Wenn weder Tages-Plan noch heute-geplant greift,
    // fällt der Hero stillschweigend auf `currentWorkoutPreview` zurück
    // (= savedWorkoutPlans.first). Der Eyebrow soll dann nicht „KRAFT · HEUTE"
    // versprechen, sondern „VORSCHLAG" markieren — sonst denkt der User, das
    // sei sein zugewiesener Plan.
    let isFallbackPreview = day.workoutPlan == nil && store.todayPlannedWorkout == nil

    GainsHeroCard(
      eyebrow: [
        isRunDay ? "LAUFEN" : "KRAFT",
        isFallbackPreview && !isRunDay ? "VORSCHLAG" : "HEUTE",
  ...
  private func heroMetricsList(day: WorkoutDayPlan, plan: WorkoutPlan) -> [GainsHeroMetric] {
    ...
    case .flexible:
      // 2026-05-10 G-P2-4: 3. Slot war `plan.split` — verwirrend, weil das
      // den fallback-Plan-Split nennt, obwohl der Tag flexibel ist. Stattdessen
      // den nächsten echten Trainingstag aus der Wochenstruktur ziehen, damit
      // der User sieht, was als Nächstes ansteht.
      return [
        .init("SESSIONS", "\(store.weeklySessionsCompleted)/\(store.weeklyGoalCount)"),
        .init("VOLUMEN",  String(format: "%.1f t", store.weeklyVolumeTons)),
        .init("NÄCHSTES", nextPlannedLabel),
      ]
    }
  }

  /// G-P2-4: Kompakter Hint auf den nächsten geplanten Trainingstag in der
  /// laufenden Wochenstruktur. Gibt „—" zurück, wenn überhaupt nichts mehr
  /// kommt (komplett leerer Plan).
  private var nextPlannedLabel: String {
    let schedule = store.weeklyWorkoutSchedule
    guard let todayIndex = schedule.firstIndex(where: { $0.isToday }) else {
      return schedule.first(where: { $0.status == .planned })?.weekday.shortLabel ?? "—"
    }
    for offset in 1..<schedule.count {
      let idx = (todayIndex + offset) % schedule.count
      if schedule[idx].status == .planned {
        return schedule[idx].weekday.shortLabel
      }
    }
    return "—"
  }
