...
struct WorkoutTrackerView: View {
  ...
  private var restTimerEndsAt: Date? {
    ...
    nonmutating set {
      store.activeRestTimerEndsAt = newValue
      // 2026-05-10 P1-3: Pause-State auch persistieren — sonst überlebt
      // er View-Resets, aber nicht eine Memory-Eviction durch iOS.
      store.scheduleSave()
    }
  }
  private var restDuration: Int {
    ...
    nonmutating set {
      store.activeRestDuration = newValue
      store.scheduleSave()
    }
  }
  ...
  var body: some View {
    NavigationStack {
      ...
      .onChange(of: store.activeWorkout?.id) { oldID, newID in
        ...
        isFinishing = false
        collapsedExerciseIDs.removeAll()
        lastAutoCollapsedID = nil
      }
      .onChange(of: store.activeWorkout?.exercises.flatMap { $0.sets }.map { $0.id } ?? []) { _, setIDs in
        guard let activeSetID, !setIDs.contains(activeSetID) else { return }
        stopActiveSet()
      }
      .onChange(of: store.activeWorkout?.exercises.map { $0.id } ?? []) { _, exerciseIDs in
        collapsedExerciseIDs = collapsedExerciseIDs.intersection(Set(exerciseIDs))
        if let skipConfirmExercise, !exerciseIDs.contains(skipConfirmExercise.id) {
          self.skipConfirmExercise = nil
        }
        if let lastAutoCollapsedID, !exerciseIDs.contains(lastAutoCollapsedID) {
          self.lastAutoCollapsedID = nil
        }
      }
      .onDisappear {
  ...
  private func exerciseCard(
            ...
            CompactSetRow(
              ...
              onDuplicate: {
                ...
                if store.duplicateSet(exerciseID: exercise.id, setID: set.id) {
                  revealExerciseForNewOpenSet(exercise.id)
    ...
    .onChange(of: isAllDone) { _, allDone in
      if !allDone {
        if lastAutoCollapsedID == exercise.id {
          lastAutoCollapsedID = nil
        }
        return
      }
      guard lastAutoCollapsedID != exercise.id else { return }
  ...
  /// 2026-05-10: Aus dem confirmationDialog-Title rausgezogen, weil der Inline-
  /// Map-Ausdruck den Type-Checker im Body-Kontext über die Reasonable-Time-
  /// Schwelle gezogen hat (Build-Fehler nach Merge mit anderen Tracker-Patches).
  private var skipConfirmTitle: String {
    if let exercise = skipConfirmExercise {
      return "'\(exercise.name)' überspringen?"
    }
    return "Übung überspringen?"
  }

  /// 2026-05-10: Body-ZStack aus dem `var body` extrahiert. Mit allen
  /// Modifier-Chains zusammen kippt Swifts Type-Checker bei Compile-Time mit
  /// „expression too complex". Ein separater Computed-Property entkoppelt die
  /// Inferenz und hält den Build grün.
  @ViewBuilder
  ...
  private var emptyTrackerState: some View {
  ...
  private func revealExerciseForNewOpenSet(_ exerciseID: UUID) {
    collapsedExerciseIDs.remove(exerciseID)
    if lastAutoCollapsedID == exerciseID {
      lastAutoCollapsedID = nil
    }
  }
