...
struct GymStatsTab: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.xl) {
      ...
      if !store.exerciseStrengthProgress.isEmpty || !store.workoutHistory.isEmpty {
        ...
        prSection
        if !filteredStrengthProgress.isEmpty {
  ...
  /// 2026-05-10 G-P1-8: Strength-Progress respektiert jetzt den Zeitraum-Filter.
  private var filteredStrengthProgress: [ExerciseStrengthProgress] {
    store.exerciseStrengthProgress(from: filteredHistory)
  }
  ...
  private var strengthProgressSection: some View {
    VStack(alignment: .leading, spacing: GainsSpacing.m) {
      ...
      LazyVGrid(
        ...
        ForEach(filteredStrengthProgress) { item in
          Button {
