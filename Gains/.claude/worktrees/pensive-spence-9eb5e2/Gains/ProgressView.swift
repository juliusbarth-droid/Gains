...
struct ProgressContentView: View {
  ...
  private var historySection: some View {
    let merged = mergedHistory(filter: activeHistoryFilter)
    let availableFilters = visibleHistoryFilters
    ...
    return VStack(alignment: .leading, spacing: GainsSpacing.s) {
      HStack(alignment: .top, spacing: GainsSpacing.s) {
        VStack(alignment: .leading, spacing: 2) {
          ...
          if let summary = historyVisibleSummary(total: merged.count, filter: activeHistoryFilter) {
            Text(summary)
              .font(GainsFont.label(9))
              .tracking(0.8)
              .foregroundStyle(GainsColor.softInk)
            ...
            ForEach(availableFilters) { filter in
              Button {
                ...
                Text(filter.label)
                  .font(GainsFont.label(9))
                  .tracking(1.0)
                  .foregroundStyle(activeHistoryFilter == filter ? GainsColor.onLime : GainsColor.softInk)
                  .padding(.horizontal, GainsSpacing.tight)
                  .frame(height: 24)
                  .background(activeHistoryFilter == filter ? GainsColor.lime : GainsColor.card)
                  .clipShape(Capsule())
              }
              .buttonStyle(.plain)
            }
  ...
  private func historyTitle(_ entry: HistoryEntry) -> String {
    ...
    case .workout(let w):
      ...
      return trimmed.isEmpty ? "Krafttraining" : trimmed
    case .run(let r):
      let trimmed = r.title.trimmingCharacters(in: .whitespacesAndNewlines)
      if !trimmed.isEmpty { return trimmed }
      let routeName = r.routeName.trimmingCharacters(in: .whitespacesAndNewlines)
      return routeName.isEmpty ? "Cardio-Session" : routeName
    }
  }
  ...
  private func historySubtitle(_ entry: HistoryEntry) -> String {
    switch entry {
    case .workout(let w):
      if w.completedSets <= 0, w.volume <= 0 {
        return "Abgeschlossenes Krafttraining"
      }
      if w.volume <= 0 {
        let setLabel = w.completedSets == 1 ? "Satz" : "Sätze"
        return "\(w.completedSets) \(setLabel) abgeschlossen"
      }
      let volumeLabel = w.volume < 1000
        ? String(format: "%.0f kg", w.volume)
        : String(format: "%.1f t", w.volume / 1000)
      if w.completedSets <= 0 {
        return "\(volumeLabel) bewegt"
      }
      let setLabel = w.completedSets == 1 ? "Satz" : "Sätze"
      return "\(volumeLabel) · \(w.completedSets) \(setLabel)"
    case .run(let r):
      if r.distanceKm <= 0 {
        if r.durationMinutes > 0 {
          return String(format: "%d Min · Cardio abgeschlossen", r.durationMinutes)
        }
        return "Abgeschlossene Cardio-Session"
      }
      if r.averagePaceSeconds <= 0 {
        if r.durationMinutes > 0 {
          return "\(String(format: "%.1f km", r.distanceKm)) · \(r.durationMinutes) Min"
        }
        return "\(String(format: "%.1f km", r.distanceKm)) · Cardio abgeschlossen"
      }
  ...
  private var visibleHistoryFilters: [HistoryFilter] {
    var filters: [HistoryFilter] = [.all]
    if !store.workoutHistory.isEmpty { filters.append(.strength) }
    if !store.runHistory.isEmpty { filters.append(.cardio) }
    return filters
  }

  private var activeHistoryFilter: HistoryFilter {
    visibleHistoryFilters.contains(historyFilter) ? historyFilter : .all
  }
  ...
  private var historyEmptyTitle: String {
    ...
    case .strength:
      return "Noch keine Kraft-Einheiten"
    case .cardio:
      return "Noch keine Cardio-Einheiten"
    }
  }
  ...
  private var historyEmptyMessage: String {
    switch activeHistoryFilter {
    case .all:
      return "Sobald du Trainings oder Cardio-Sessions abschließt, sammeln sich hier deine letzten Aktivitäten."
    case .strength:
      return "In diesem Verlauf sind noch keine abgeschlossenen Kraft-Einheiten sichtbar. Wechsle auf Alle oder Cardio, um andere Aktivitäten zu sehen."
    case .cardio:
      return "In diesem Verlauf sind noch keine abgeschlossenen Cardio-Einheiten sichtbar. Wechsle auf Alle oder Kraft, um andere Aktivitäten zu sehen."
    }
  }

  private func historyVisibleSummary(total: Int, filter: HistoryFilter) -> String? {
    guard total > 0 else { return nil }
    switch filter {
    case .all:
      if total > 5 {
        return "Neueste 5 von \(total)"
      }
      let label = total == 1 ? "Aktivität" : "Aktivitäten"
      return "\(total) \(label)"
    case .strength:
      if total > 5 {
        return "Neueste 5 Kraft-Einheiten von \(total)"
      }
      let label = total == 1 ? "Kraft-Einheit" : "Kraft-Einheiten"
      return "\(total) \(label)"
    case .cardio:
      if total > 5 {
        return "Neueste 5 Cardio-Einheiten von \(total)"
      }
      let label = total == 1 ? "Cardio-Einheit" : "Cardio-Einheiten"
      return "\(total) \(label)"
    }
  }
