...
struct GymWorkoutsTab: View {
  ...
  private func apply(searchText: String, to plans: [WorkoutPlan]) -> [WorkoutPlan] {
    ...
    // 2026-05-10 G-P2-7: Suche zieht jetzt auch die Übungs-Namen mit. Vorher
    // matchte „Bench Press" auf nichts, weil dieser Bench-Plan im Titel
    // „Push A" hieß — die Übungs-Bibliothek war von der Suche entkoppelt.
    return plans.filter { plan in
      if plan.title.localizedCaseInsensitiveContains(trimmed) { return true }
      if plan.split.localizedCaseInsensitiveContains(trimmed) { return true }
      if plan.focus.localizedCaseInsensitiveContains(trimmed) { return true }
      return plan.exercises.contains {
