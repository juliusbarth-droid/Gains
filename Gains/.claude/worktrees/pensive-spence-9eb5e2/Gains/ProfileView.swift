...
struct ProfileView: View {
  ...
  // 2026-05-10 #28: Wenn User Notifications ON tippt aber System schon
  // .denied gespeichert hat, kommt kein iOS-Prompt mehr. Zeige stattdessen
  // einen Alert mit Settings-Deep-Link, damit der Toggle nicht ins Leere
  // greift.
  ...
  private var optionsCard: some View {
      ...
      VStack(spacing: 0) {
        toggleRow(
          ...
          binding: Binding(
            ...
            set: { newValue in
              // 2026-05-10 #28: Wenn der User aktivieren will, erst System-Status
              // checken. Bei .denied zeigt iOS das Permission-Prompt nicht erneut —
              // der App-Toggle wäre an, aber kein Reminder käme an. In dem Fall
              // statt stillem No-Op einen Settings-Deep-Link-Alert anbieten.
              if newValue && !store.notificationsEnabled {
                NotificationsManager.shared.currentAuthorization { status in
                  if status == .denied {
    ...
    // 2026-05-10 #28: Settings-Deep-Link, wenn iOS-Permission .denied ist und
    // der App-Toggle aktiviert werden soll. Sonst hätte der Toggle visuell „on"
    // gestanden, ohne dass je eine Notification kommt.
    .alert(
      "Mitteilungen sind deaktiviert",
      isPresented: $showsNotificationsDeniedAlert
    ) {
      ...
      Text(
        "Du hast Mitteilungen für Gains in den iOS-Einstellungen deaktiviert. Aktiviere sie dort, damit Workout-Reminder ankommen können."
      )
    }
  }
